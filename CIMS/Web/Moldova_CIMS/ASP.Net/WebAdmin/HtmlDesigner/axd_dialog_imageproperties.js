...
if(c){if(b){if(typeof(MutationObserver)!="undefined"){this.styleDisplayObserver=new MutationObserver(Function.createDelegate(this,this._styleDisplayObserverCallback));
this.styleDisplayObserver.observe(b,{attributes:true,attributeOldValue:true,attributeFilter:["style","class"],subtree:false});
}else{if($telerik.isIE){$telerik.addHandler(b,"propertychange",a);
}else{b.addEventListener("DOMAttrModified",a,false);
}}}}else{if(b&&a){if(this.styleDisplayObserver){this.styleDisplayObserver.disconnect();
}else{if($telerik.isIE){$telerik.removeHandler(b,"propertychange",a);
...
if(a.disabled&&$telerik.isIE6){Sys.UI.DomElement.addCssClass(a,"rfdInputDisabled");
}},decorateTextArea:function(a){if(a.disabled&&$telerik.isIE6){Sys.UI.DomElement.addCssClass(a,"rfdInputDisabled");
...
/* START Telerik.Web.UI.Dialogs.DialogInitializerScripts.js */
...
/* END Telerik.Web.UI.Dialogs.DialogInitializerScripts.js */
...
}},_setCursorInNode:function(g,f,k,e){var h=k.get_document();
var d=h.createElement("br"),n;
a(d).insertAfter(g);
a(g).remove();
if($telerik.isSafari||$telerik.isOpera||$telerik.isFirefox){d.parentNode.normalize();
k.getSelection().moveToElementText(d.parentNode);
if($telerik.isSafari&&!$telerik.isChrome){n=h.createTextNode("");
a(n).insertBefore(d);
k.getSelection().moveToElementText(d.parentNode);
}if($telerik.isFirefox&&f.nodeName=="LI"&&a(f).find("ol,ul").length==0){a(d).remove();
}}else{var l=h.createElement("p"),m;
l.innerHTML="&nbsp;";
n=l.firstChild;
a(n).insertBefore(d);
if($telerik.isIE9Mode){a(d).remove();
m=k.getSelection().getRange(true);
var i=Telerik.Web.UI.Editor.DomRange.toDomRange(m);
this._setCursorInTextNode(i,n,0,1);
}else{m=k.getSelection().getRange(true);
m.moveToElementText(f);
m.collapse(true);
m.select();
a(d).remove();
}h.execCommand("delete",false,null);
}},_setCursorInTextNode:function(d,g,f,e){if(!d.range){d=Telerik.Web.UI.Editor.DomRange.toDomRange(d);
}d.setStart(g,f);
d.setEnd(g,e);
d.setStart(g,f);
if(f==e){d.collapse(!$telerik.isIE||$telerik.isIE9Mode);
}d.select();
},_setCursorAroundList:function(e,d,g,i){var f=g.get_document();
if($telerik.isFirefox){if(e.nextSibling&&e.nextSibling.nodeName=="BR"){a(e.nextSibling).remove();
}this._setCursorInNode(e,d,g,i);
}else{if($telerik.isSafari||$telerik.isOpera){textNode=e.firstChild;
if($telerik.isSafari){br=f.createElement("br");
a(br).insertAfter(e);
a(e).remove();
var h=br.parentNode;
if(br.nextSibling&&br.nextSibling.nodeName=="BR"){a(br).remove();
}g.getSelection().moveToElementText(h);
}else{this.removeNode(e);
g.getSelection().moveToElementText(textNode);
textNode.nodeValue="";
}}else{br=f.createElement("br");
a(br).insertAfter(e);
textNode=f.createTextNode(" ");
a(textNode).insertBefore(br);
this._setCursorInTextNode(i,textNode,0,0);
a(br).remove();
}}a(e).remove();
...
}},getElementParentByRegExpTagName:function(f,g){if(null==f){return null;
}if(null==g){return f;
}try{while(f&&null!=f.tagName&&!g.test(f.tagName)){if(b.Utils.isEditorContentArea(f)){break;
}f=f.parentNode;
}return((g.test(f.tagName))?f:null);
}catch(d){return null;
...
},getPreviousElement:function(d){if(d.previousElementSibling){return d.previousElementSibling;
}var e=a(d).prev();
if(e.length>0&&!this.isTextNode(e[0])){return e[0];
}return null;
},getNextElement:function(e){if(e.nextElementSibling){return e.nextElementSibling;
}var d=a(e).next();
if(d.length>0&&!this.isTextNode(d[0])){return d[0];
}return null;
...
},isBlockElement:function(d){return this.checkForElement(d,/^(?:body|p|div|h[1-6]|form|fieldset|table|thead|tbody|tr|th|td|ul|ol|li|dl|blockquote|address|hr)$/i);
...
if(e){d.nodeValue+=" ";
}},trimElementNeighbours:function(d){var f=d.previousSibling,e=d.nextSibling;
if($telerik.isIE&&!$telerik.isIE9Mode){var g=/^[\s\t\r\n\u200b\u00a0]+$/;
if(f&&this.isTextNode(f)&&g.test(f.nodeValue)){f.nodeValue="";
}if(e&&this.isTextNode(e)&&g.test(e.nodeValue)){e.nodeValue="";
}}if(f){this.trimWhitespace(f);
}if(e){this.trimWhitespace(e);
...
},isEmptyDom:function(d){return this.isTextNode(d)?this.isTextNodeEmpty(d):this.isNodeEmpty(d);
},isNodeEmptyRecursive:function(e){if(e.childNodes.length==0){return this.isEmptyDom(e);
}var d=e.firstChild;
while(d&&this.isNodeEmptyRecursive(d)){d=d.nextSibling;
}return !d;
...
return true;
},OnExecuteIE:function(){if($telerik.isIE10Mode&&!$telerik.isIE10){return this.OnExecuteMoz();
...
Telerik.Web.UI.Editor.EnterNewLineCommand.insertBrAfterHeading=false;
...
}var b=new Telerik.Web.UI.Editor.GenericCommand("Enter",a.get_contentWindow(),a);
this.deleteContents();
var c=true;
switch(this._newLineMode){case Telerik.Web.UI.EditorNewLineModes.P:case Telerik.Web.UI.EditorNewLineModes.Div:c=this.enterBlockElement();
break;
default:c=this.enterBr();
break;
}if($telerik.isSafari||$telerik.isFirefox){this.moveScroll(a);
}a.executeCommand(b);
return c;
},deleteContents:function(){var a=this.editor.getSelection().getRange();
if(a&&a.collapsed===false||!!a.text){this.editor.get_document().execCommand("delete",false,null);
}},moveScroll:function(){var a=this.editor,i=a.get_contentArea().scrollTop,c=a.get_newLineMode(),d=Telerik.Web.UI.EditorNewLineModes;
try{var f=null,b=0;
if(c==d.Br){a.pasteHtml("<span id='_radEditor_new_line'>&nbsp;</span>","",null,null,false);
f=a.get_document().getElementById("_radEditor_new_line");
b=f.offsetHeight+1;
}else{f=a.getSelectedElement();
if(!f){return;
}var g=this.utils.getElementParentByTag(f,"P");
var e=this.utils.getElementParentByTag(f,"DIV");
if((c!=d.Div&&!g)||(c==d.Div&&!e)){return;
}b=(c==d.Div)?e.offsetHeight:g.offsetHeight+20;
}if(f.offsetTop-i+5>a.get_contentAreaElement().offsetHeight){a.get_contentArea().scrollTop=i+b;
}if(f&&f.id=="_radEditor_new_line"){var h=f.previousSibling;
if(h){f.parentNode.removeChild(f);
a.selectElement(h);
}else{a.selectElement(f);
...
var f=a.getSelection().getRange();
var g=a.getSelectedElement();
var e=this.utils.getElementParentByRegExpTagName(g,/^p|s|u|i|strike|strong|span|font|div|sup|sub$/i);
var c=this.utils.getElementParentByTag(g,"LI");
var d=this.utils.getElementParentByTag(g,"A");
var b=this.utils.getElementParentByRegExpTagName(g,this.regExpHeader);
if(c){if(this.shouldOutdentListItem_OnEnter(c)){return this.outdentListItem_OnEnter(c);
}else{if(this.shouldDiscontinueList(g)){return this.discontinueList_OnEnter(g);
}else{if(c&&$telerik.$(c).find("ol,ul").length>0){return this.executeInsertParagraphCommand(a,null);
}else{return this.executeInsertParagraphCommand(a,c);
}}}}if(b){this.insertBrInHeading(b);
}else{if(d){this.insertBrInLink(d);
}else{if(e){this._insertBrElementFirefox(f);
}else{a.bubbleKeyEventToBrowser();
}}}return true;
},enterBrIE:function(){var c=this.editor;
var l=c.getSelectedElement();
var f=this.utils.getElementParentByTag(l,"LI");
if(f){if(this.shouldOutdentListItem_OnEnter(f)){return this.outdentListItem_OnEnter(f);
}if(this.shouldDiscontinueList(f)){return this.discontinueList_OnEnter(f);
}if($telerik.$(f).find("ol,ul").length>0){return this.executeInsertParagraphCommand(c,null);
}c.bubbleKeyEventToBrowser();
return false;
}try{var j=c.getSelection().getRange(false);
if(j.pasteHTML){var g=j.offsetTop;
var m=l.tagName;
var k=j.duplicate();
k.moveToElementText(l);
var e=this.utils.getElementParentByRegExpTagName(l,this.regExpHeader);
var i=this.utils.getElementParentByTag(l,"A");
if(m.toLowerCase()=="textarea"){c.bubbleKeyEventToBrowser();
return false;
}else{if(e){j.pasteHTML("");
this.insertBrInHeading(e);
}else{if(m=="A"&&j.text==""&&j.compareEndPoints("StartToStart",k)==0){var h=l.parentNode;
h.insertBefore(c.get_document().createElement("BR"),l);
}else{if(i){this.insertBrInLink(i);
}else{if($telerik.isIE9Mode&&m=="BR"){j.pasteHTML("<BR /> ");
}else{j.pasteHTML("<BR />");
}if(!$telerik.isIE9Mode||g!=j.offsetTop){j.select();
j.moveEnd("character",1);
j.moveStart("character",1);
j.collapse(false);
}}}}}}else{var a=c.get_document().createElement("br");
var b=Telerik.Web.UI.Editor.DomRange.toDomRange(c.getSelection().getRange());
b.insertNode(a);
this.utils.trimElementNeighbours(a);
this.setCursorAfterBr(a,b);
}}catch(d){alert(d.message);
}return false;
},enterBrOpera:function(){var a=this.editor;
var f=a.getSelectedElement();
var c=this.utils.getElementParentByTag(f,"LI");
var d=this.utils.getElementParentByTag(f,"A");
var b=this.utils.getElementParentByRegExpTagName(f,this.regExpHeader);
if("LI"==f.tagName||c){if(this.shouldOutdentListItem_OnEnter(c)){return this.outdentListItem_OnEnter(c);
}else{if(this.shouldDiscontinueList(f)){return this.discontinueList_OnEnter(f);
}else{if($telerik.$(c).find("ol,ul").length>0){return this.executeInsertParagraphCommand(a,null);
}else{return this.executeInsertParagraphCommand(a,c);
}}}a.bubbleKeyEventToBrowser();
return false;
}else{if(b){var e=Telerik.Web.UI.Editor.DomRange.toDomRange(a.getSelection().getRange());
e.pasteHTML("");
this.insertBrInHeading(b);
}else{if(d){this.insertBrInLink(d);
}else{a.pasteHtml("<br />","",null,null,false);
selection=a.getSelection();
var e=selection.getRange();
e.collapse(false);
selection.selectRange(e);
}}}return true;
},enterBrSafari:function(){var b=this.editor;
var g=b.getSelectedElement();
var e=this.utils.getElementParentByTag(g,"LI");
var d=this.utils.getElementParentByRegExpTagName(g,this.regExpHeader);
var a=Telerik.Web.UI.Editor.DomRange.toDomRange(b.getSelection().getRange());
if(e){if(this.shouldOutdentListItem_OnEnter(e)){return this.outdentListItem_OnEnter(e);
}else{if(this.shouldDiscontinueList(g)){return this.discontinueList_OnEnter(g);
}else{if($telerik.$(e).find("ol,ul").length>0){return this.executeInsertParagraphCommand(b,null);
}else{if(this.isLastElement(g,e)&&a.endContainer&&this.utils.isTextNode(a.endContainer)&&a.endContainer.length==a.endOffset){return this.executeInsertParagraphCommand(b,e);
}else{this.executeInsertParagraphCommand(b,e);
var f=this.utils.getNextElement(e);
var c=$telerik.$("*",f)[0];
if(c){this.editor.getSelection().moveToElementText(c);
}return true;
}}}}}else{if(d){this.insertBrInHeading(d);
return true;
}}b.get_document().execCommand("InsertLineBreak",true,"");
return true;
},executeInsertParagraphCommand:function(c,b,d){var a=new Telerik.Web.UI.Editor.InsertParagraphCommand(c);
if(b){a.container=b;
}if(d===true||d===false){a.options.retainOriginalNodeName=d;
}if(d===false){a.options.tag=c.get_newLineMode()==Telerik.Web.UI.EditorNewLineModes.Div?"div":"p";
}return a.onExecute();
},insertBrInLink:function(d){var b=this.editor.getSelection().getRange(true);
if(b.collapsed===false){b.deleteContents();
}else{if(b.text&&b.text!=""){b.execCommand("Delete");
}}var c=Telerik.Web.UI.Editor.DomRange.toDomRange(b);
var a=this.editor.get_document().createElement("br");
c.insertNode(a);
if(this.isFirstElement(a,d)){$telerik.$(a).insertBefore(d);
}else{if(this.isLastElement(a,d)){$telerik.$(a).insertAfter(d);
}}this.setCursorAfterBr(a,c);
var e=this.utils.getNextElement(a);
if(e&&e.nodeName=="A"&&a.nextSibling.nodeValue&&a.nextSibling.nodeValue.trim()==""){a.nextSibling.nodeValue="";
}},insertBrInHeading:function(e){var c=this.editor.get_document(),d=Telerik.Web.UI.Editor.DomRange.toDomRange(this.editor.getSelection().getRange()),f=Telerik.Web.UI.Editor.EnterNewLineCommand.insertBrAfterHeading;
var a=c.createElement("br");
d.insertNode(a);
if(!f&&this.isLastElement(a,e)){$telerik.$(a).remove();
var g=c.createElement("p");
$telerik.$(g).insertAfter(e);
var b=c.createElement("span");
b.innerHTML="&nbsp;";
g.appendChild(b);
this.setCursorInNode(b,g,d);
}else{this.setCursorAfterBr(a,d);
}},isFirstElement:function(b,a){var c=b;
while(!$telerik.$(c).is(a)&&$telerik.$(this.editor.get_contentArea()).has(c).length!=0){while(c.previousSibling){c=c.previousSibling;
if(!this.utils.isTextNodeEmpty(c)){return false;
}}c=c.parentNode;
}return true;
},isLastElement:function(b,a){var c=b;
while(!$telerik.$(c).is(a)&&$telerik.$(this.editor.get_contentArea()).has(c).length!=0){while(c.nextSibling){c=c.nextSibling;
if(!this.utils.isTextNodeEmpty(c)){return false;
}}c=c.parentNode;
}return true;
},_insertBrElementFirefox:function(c){var d=this._window.getSelection(),b=this.editor.get_document(),a=b.createElement("br");
c.insertNode(a);
if(a.parentNode.normalize){a.parentNode.normalize();
}if(!a.nextSibling){$telerik.$(a.cloneNode()).insertAfter(a);
}c.setStartAfter(a);
c.collapse(true);
d.removeAllRanges();
d.addRange(c);
},setCursorAfterBr:function(a,d){var e,b=this.editor.get_document();
if(a.nextSibling&&this.utils.isTextNode(a.nextSibling)&&!this.utils.isTextNodeEmpty(a.nextSibling)){e=a.nextSibling;
this.setCursorInTextNode(d,e,0,0);
return;
}e=b.createTextNode("");
$telerik.$(e).insertAfter(a);
if($telerik.isSafari){$telerik.$(b.createElement("br")).insertAfter(e);
this.setCursorInTextNode(d,e,0,0);
}else{if($telerik.isOpera||$telerik.isFirefox){if($telerik.isFirefox){var c=b.createElement("p");
c.innerHTML="&nbsp;";
e.nodeValue=c.firstChild.nodeValue;
e.newLineCommand=true;
}this.setCursorInTextNode(d,e,0,0);
}else{if($telerik.isIE9Mode){var c=b.createElement("p");
c.innerHTML="&nbsp;";
e.nodeValue=c.firstChild.nodeValue;
this.setCursorInTextNode(d,e,0,0);
...
d.setStartAfter(a);
d.setEndAfter(a);
d.collapse(!$telerik.isIE||$telerik.isIE9Mode);
d.select();
}}}},setCursorInNode:function(b,a,c){this.utils._setCursorInNode(b,a,this.editor,c);
},setCursorInTextNode:function(c,d,b,a){this.utils._setCursorInTextNode(c,d,b,a);
},shouldOutdentListItem_OnEnter:function(a){return this.whiteSpaceRegExp.test(this.utils.getTextContent(a));
},shouldDiscontinueList:function(a){return this.isEmptyListItem(a)&&!this.utils.findAncestorBy(this.editor.get_contentArea(),a.parentNode,this.isList);
},isEmptyListItem:function(a){return this.utils.isTag(a,"li")&&(a.innerHTML=="<br>"||this.utils.isNodeEmpty(a)||(a.childNodes.length==1&&this.utils.isTextNodeEmpty(a.firstChild))||(a.childNodes.length==2&&this.utils.isTextNodeEmpty(a.firstChild)&&this.utils.isTag(a.lastChild,"br"))||(a.childNodes.length==3&&this.utils.isTextNodeEmpty(a.firstChild)&&this.utils.isTextNodeEmpty(a.childNodes[1])&&this.utils.isTag(a.lastChild,"br")));
},isList:function(a){return Telerik.Web.UI.Editor.Utils.isTag(a,"UL")||Telerik.Web.UI.Editor.Utils.isTag(a,"OL");
},outdentListItem_OnEnter:function(j){var d=this.editor,c=this.editor.get_document(),i=j.parentNode,o=this.editor.getSelection().getRange(),a;
if(this.utils.getElementParentByTag(i,"LI")){a=this.insertCursor(o,c);
try{var n=new Telerik.Web.UI.Editor.OutdentCommand(d);
n.onExecute();
}catch(f){}this._clearList(i);
this.utils._setCursorAroundList(a,j,d,o);
}else{if($telerik.isIE&&!$telerik.isIE9Mode){d.get_document().execCommand("Outdent");
o=this.editor.getSelection().getRange();
if(d.get_newLineMode()==Telerik.Web.UI.EditorNewLineModes.Div){var e=o.parentElement(),p=c.createTextNode("\u00A0");
e.appendChild(p);
var g=this.utils.getElementParentByTag(e,"P"),b=g.cloneNode(true);
b=this.utils.replaceNode(b,"div");
$telerik.$(b).insertAfter(g);
$telerik.$(g).remove();
d.selectElement(b);
o.collapse(true);
}o.select();
return true;
}var h=$telerik.$(j).index();
a=this.insertCursor(o,c);
if(h!==i.children.length-1){var k=i.cloneNode(true);
$telerik.$(k).insertAfter(i);
$telerik.$(i).children().filter(function(){return $telerik.$(this).index()>h;
}).remove();
$telerik.$(k).children().filter(function(){return $telerik.$(this).index()<=h;
}).remove();
}$telerik.$(j).insertAfter(i);
var m=d.get_newLineMode()==Telerik.Web.UI.EditorNewLineModes.Div?"div":"p";
var l=this.utils.replaceNode(j,m);
this._clearList(i);
$telerik.$(l).find("br").remove();
this.setCursorInNode(a,l,o);
}return true;
},_clearList:function(c){for(var b=c.childNodes.length-1;
b>=0;
b--){var a=c.childNodes[b];
if(a.nodeName!="LI"){c.removeChild(a);
}}if(c.children.length==0){$telerik.$(c).remove();
}},insertCursor:function(d,c){var b="__TELERIK_NEWLINE_";
var a=c.createElement("span");
a.id=b;
a.innerHTML="&nbsp;";
if(d.insertNode){d.insertNode(a);
}else{d.pasteHTML(this.utils.getOuterHtml(a));
a=this.editor.get_document().getElementById(b);
}a.removeAttribute("id");
return a;
...
a.onExecute();
return true;
},enterBlockElement:function(){var a=this.editor;
var d=a.getSelectedElement();
var b=this.utils.getElementParentByRegExpTagName(d,this.regExpHeader);
var c=this.utils.getElementParentByTag(d,"LI");
if(c){if(this.shouldOutdentListItem_OnEnter(c)){return this.outdentListItem_OnEnter(c);
}else{return this.executeInsertParagraphCommand(a,c,false);
}}else{if(b){return this.enterHeading(b);
}else{return this.executeInsertParagraphCommand(a,null,false);
}}},enterHeading:function(b){var a=new Telerik.Web.UI.Editor.InsertParagraphCommand(this.editor);
a.container=b;
a.options.insertParagraphAfterHeading=true;
a.options.retainOriginalNodeName=false;
a.options.tag=this.editor.get_newLineMode()==Telerik.Web.UI.EditorNewLineModes.Div?"div":"p";
return a.onExecute();
},regExpHeader:/^h[1-6]$/i,isHeader:function(a){return this.regExpHeader.test(a.nodeName);
}};
...
},setToolState:function(k,e){if(!k){k=this._tools;
}var c=this.get_editor(),g=this._rangeState||b.Editor.DomRange.rangeFromWindow(c.get_contentWindow());
for(var d=0;
d<k.length;
d++){var f=k[d];
if(f.get_name){var j=f.get_name();
if(f.setState){var h=e;
if(null==h){h=c.getToolState(j,g);
}if(null!=h){f.setState(h);
}}if(f.updateValue){f.updateValue(c.getToolValue(j));
...
}}},_onBodyMouseDown:function(d){var g=Telerik.Web.UI.Editor.Utils,c=this.get_editor().get_contentArea(),f=d.srcElement||d.target;
this._mouseIsDownOnContentArea=$telerik.isDescendantOrSelf(c,f)||g.isEditorEmptyMessage(f);
},_onBodyClick:function(c){if(this.get_element()&&this.get_element().style.visibility=="hidden"){return;
}var d=this.get_editor(),h=this.get_toolbarHolder(),g=h?h.get_popupElement():null,f=c.srcElement||c.originalTarget;
...
this.setToolState();
this._rangeState=null;
},setRibbonBarToolState:function(h,c){if(!h){h=this._ribbonBarButtons;
}var a=this.get_editor(),e=Telerik.Web.UI.Editor.DomRange.rangeFromWindow(a.get_contentWindow());
this._rangeState=e;
for(var b=0;
b<h.length;
b++){var d=h[b];
var g=d.get_value();
var f=c;
if(null==f){f=a.getToolState(g,e);
}if(null!=f){this._setRibbonBarButtonState(d,f);
...
Type.registerNamespace("Telerik.Web.UI.Editor");
(function(a,b,d,c){b.DomBlockCommandBase=function(e,g){var f=this.options=a.extend({window:e?e.get_contentWindow():c,title:"Indent",canUnexecute:c,blockTag:"p",noDuplicateNodeEdges:true},g||{});
...
},inlineWrapper:null,executeOnNode:Function.emptyFunction,executeInlineNode:function(f,g){var e=this.collectFragment(f);
if(g||!this.isEmptyFragment(e)){this.executeOnFragment(e);
}},executeOnFragment:function(e){var f=this.createWrappingBlock();
this.wrapFragmentInBlock(e,f);
this.executeOnNode(f);
return f;
},getCssState:function(i,e,f){var g=this.nodes=this.collectElements(i);
...
var e=i.traverse(h);
this.removeDuplicateEdges(e);
if(e.length==1&&d.isTag(e[0],"br")){var f=e[0];
e=[f.previousSibling||f.nextSibling];
}return e;
},findFirstInlineChild:function(f){var e=f.firstChild;
while(d.isBlockElement(e)){e=e.nextSibling;
}return e;
},removeDuplicateEdges:function(g){if(!this.options.noDuplicateNodeEdges||g.length==0){return;
}var e=g[0],f=g[g.length-1];
if(g.length>1&&e==f){g.pop();
...
(function(a,b,d,c){b.InlineCommand=function(f,e,h){this.settings=e;
var g=this.options=a.extend({window:f?f.get_contentWindow():c,title:"Inline command",canUnexecute:c},h||{});
b.InlineCommand.initializeBase(this,[g.title,g.window,g.canUnexecute,f]);
};
b.InlineCommand.prototype={onExecute:function(){var e=this,g=e.range=e.getEditorRange(),h=e.getState(e.get_editor().get_contentWindow()),f=e.traverseFragments(g);
e.emptyRange=g.toString()=="";
e.storeRangeByFragments(f);
e.rangeChanged=false;
if(h==b.CommandStates.On){e.removeFormat(f);
}else{e.formatFragments(f);
}if(!e.rangeChanged){e.restoreRange();
}return true;
},getState:function(h,e,g){var f=g||b.DomRange.rangeFromWindow(h);
return this.getStateByRange(f);
},getStateByRange:function(e){if(!e){return b.CommandStates.Off;
}if(!e.isCollapsed()){e.normalizeEdges();
}var f=e.startContainer;
if(!d.isTextNode(f)&&f.childNodes[e.startOffset]){f=f.childNodes[e.startOffset];
}f=this.findFormattedAncestor(f);
return this.isSameFormatNode(f)?b.CommandStates.On:b.CommandStates.Off;
},removeFormat:function(k){var e=this,g=e.get_editor();
if(e.emptyRange){var m=b.DomRange.rangeFromWindow(g.get_contentWindow()),i=e.findFormattedAncestor(m.startContainer);
if(this.isRangeAtEnd(m,i)){var f=e.get_editor().get_document(),n=d.isTextNode(m.startContainer)?m.startContainer.parentNode:m.startContainer,l=f.createElement("span");
l.className="__telerik_marker";
l.innerHTML="&nbsp;";
var o=l.cloneNode(true),h=l;
n.appendChild(o);
n.appendChild(h);
e.removeFormat_extract(i,o,h);
}else{e.persistRangeThroughParent(i);
e.removeNodeFormatting(i);
}}else{while(k.length){var j=k.shift();
if(!j.isEmpty()){this.removeFormat_fragment(j);
}}}},isRangeAtEnd:function(g,f){var h=g.startContainer,i=g.startOffset,e=d.isTextNode(h)?i==h.length:i==h.childNodes.length;
while(e&&d.isAncestorOf(f,h)){while(e&&h.nextSibling){e=d.isEmptyDom(h);
h=h.nextSibling;
}h=h.parentNode;
}return e;
},removeFormat_fragment:function(g){this.cleanUpFormat(g);
var j=g.getParent(),f=this.findFormattedAncestor(j);
if(!this.isSameFormatNode(f)){return;
}var i=g.nodes,e=i[0],h=i[i.length-1];
this.removeFormat_extract(f,e,h);
},removeFormat_extract:function(g,f,i){var h=g.parentNode,e=new b.DomTreeExtractor(g),j=e.extractBefore(f),l=e.extractAfter(i),k=this.cloneNodeClean(g),m=this.cloneNodeClean(g);
m.appendChild(l);
this.removeSameFormatChildren(m);
k.appendChild(j);
this.removeSameFormatChildren(k);
if(!d.isNodeEmptyRecursive(k)){h.insertBefore(k,g);
}if(!d.isNodeEmptyRecursive(m)){h.insertBefore(m,g.nextSibling);
}this.removeNodeFormatting(g);
this.removeEmptyNode(g);
},removeNodeFormatting:function(f){var g=this,e=g.isSameTagFormat(f);
this.removeNodeCssFormatting(f);
if(e){f=d.replaceNode(f,"span");
}if(!g.isNodeStyled(f)&&d.isTag(f,"span")){d.removeNode(f);
}},removeNodeCssFormatting:function(g){var h=this,f=h.settings;
if(h.isSameCssFormat(g)){var i=g.style,j=i[f.cssName].replace(f.cssValue,"").trim();
i[f.cssName]=j;
if($telerik.isIE6||$telerik.isIE7||$telerik.isIE8){var e=i.cssText+";";
i.cssText=e.replace(/((font-family)|(text-decoration)):\s*;/gi,"");
}}},formatFragments:function(f){if(f.length==0){this.insertSingleFormatNode();
}while(f.length){var e=f.shift();
if(!e.isEmpty()){this.formatFragment(e);
}}},insertSingleFormatNode:function(){range=this.getEditorRange();
var e=this.createInlineNode();
range.insertNode(e);
this.get_editor().setActive();
this.get_editor().setFocus();
this.setCursorInInlineNode(e);
},formatFragment:function(f){this.cleanUpFormat(f);
if(this.isSameFormat(f)){if(this.formatNode&&this.isParentCommon(f.getParent(),this.formatNode)){var e=f.getParent();
f.clear();
f.addAll(e.childNodes);
f.appendTo(this.formatNode);
e.parentNode.removeChild(e);
}}else{formatNode=this.formatNode=this.createInlineNode();
f.insertBeforeFirst(formatNode);
f.appendTo(formatNode);
}},createInlineNode:function(){var e=this.get_window().document,f=this.settings,h=!f.tag,g=e.createElement(f.tag||"span");
if(h){g.style[f.cssName]=f.cssValue;
}return g;
},cloneNodeClean:function(f){var e=f.cloneNode(false);
e.innerHTML="";
return e;
},traverseFragments:function(e){if(!e){e=this.getEditorRange();
}var g=new Telerik.Web.UI.Editor.RangeFragmentsTraverser(e);
var f=b.FragmentsCondition.isInlineNode;
g.traverseFragments(f);
return g.traversedFragments;
},storeRangeByFragments:function(g){if(g.length){var f=g[0];
var h=g[g.length-1];
var e=this.get_editor().get_document();
var i=e.createElement("span");
i.className="__telerik_marker";
i.innerHTML="&nbsp;";
this.startMarker=i.cloneNode(true);
this.endMarker=i;
f.insertBeforeFirst(this.startMarker);
f.addNodeAt(this.startMarker,0);
h.appendNodeAfter(this.endMarker);
h.addNode(this.endMarker);
}else{this.storeRange();
}},storeRange:function(e){var e=e||this.getEditorRange();
this.rangeMemento=new b.DomRangeMemento(e);
},persistRangeThroughParent:function(g){var f=this.rangeMemento;
if(f){var e=d.findNodeIndex(g);
if(f.startContainer==g){f.startContainer=g.parentNode;
f.startOffset+=e;
}if(f.endContainer==g){f.endContainer=g.parentNode;
f.endOffset+=e;
}}},restoreRange:function(){var n=this.range;
var k=$telerik.$(".__telerik_marker",this.get_editor().get_contentArea());
if(k.length){var i=k[0],l=i.nextSibling,j=k[1];
if((d.isTextNode(l)&&d.isTextNodeEmpty(l)&&j.previousSibling===l)||(i===j.previousSibling)){var g=this.get_editor(),m=i.parentNode;
g.setActive();
g.setFocus();
...
n.collapse();
n.select();
g.whitespaceTracker.trackNode(h);
}else{n.setStartAfter(k[0]);
n.setEndBefore(k[1]);
n.select();
k.remove();
}}else{try{if(this.rangeMemento){this.rangeMemento.restoreToRange(this.getEditorRange());
}}catch(f){}}},getEditorRange:function(){return b.DomRange.toDomRange(this.get_editor().getSelection().getRange(true));
},removeEmptyNode:function(e){if(d.isNodeEmptyRecursive(e)&&e.parentNode){e.parentNode.removeChild(e);
}},cleanUpFormat:function(e){this.cleanUpFragment=e;
var g=e.nodes;
for(var f=g.length-1;
f>=0;
f--){this.findFormat(g[f],f);
}},findFormat:function(f,e){if(d.isTextNode(f)){return;
}this.removeSameFormatChildren(f);
this.removeSameFormat(f,e);
},removeSameFormatChildren:function(e){var f=a.makeArray(e.getElementsByTagName(this.settings.tag));
while(f.length){this.removeSameFormat(f.shift());
}},removeSameFormat:function(k,g){if(this.isSameFormatNode(k)){if(!isNaN(g)){this.cleanUpFragment.removeNodeAt(g);
var f=k.childNodes;
for(var h=f.length-1;
h>=0;
h--){var e=f[h];
this.cleanUpFragment.addNodeAt(e,g);
}}this.removeNodeFormatting(k);
}},findFormattedAncestor:function(g){if(this.isSameFormatNode(g)){return g;
}var f=g.ownerDocument;
var h=f.getElementsByTagName("body")[0];
var e=Function.createDelegate(this,function(i){return this.isSameFormatNode(i)||d.isEditorContentArea(i);
});
return d.findAncestorBy(h,g,e);
},isSameFormat:function(e){var f=e.getParent();
return this.isSameFormatNode(f);
},isSameFormatNode:function(e){return this.isSameTagFormat(e)||this.isSameCssFormat(e);
},isSameTagFormat:function(e){return(d.isTag(e,this.settings.tag)||this.isAltTagFormat(e))&&!e.className;
},isAltTagFormat:function(f){var g=this.settings.altTags;
for(var e=0;
e<g.length;
e++){if(d.isTag(f,g[e])){return true;
}}return false;
},isSameCssFormat:function(f){var g=this.settings,e=g.cssName,h=g.cssValue;
return f.style&&f.style[e].indexOf(h)>-1;
},isNodeStyled:function(e){return e.style.cssText!="";
},isParentCommon:function(e,f){return e&&f&&e.parentNode&&e.parentNode==f.parentNode;
},setCursorInInlineNode:function(f){f.innerHTML="&#x200b;";
var e=f.firstChild;
this.get_editor().whitespaceTracker.trackNode(e);
range.setStart(f,1);
range.setEnd(f,1);
range.collapse();
range.select();
...
var g=b[f]=function(k,l){return new b.InlineCommand(k,h,l);
};
g.registerClass(j);
}b.InlineCommandSettings={Bold:{tag:"strong",altTags:["b"],cssName:"fontWeight",cssValue:"bold"},Italic:{tag:"em",altTags:["i"],cssName:"fontStyle",cssValue:"italic"},Underline:{altTags:["u"],cssName:"textDecoration",cssValue:"underline"},StrikeThrough:{tag:"del",altTags:["strike"],cssName:"textDecoration",cssValue:"line-through"}};
c("Bold");
c("Italic");
c("Underline");
c("StrikeThrough");
...
this.utils=b.Utils;
this.linkProperties={id:h.getAttribute("id"),href:h.getAttribute("href"),target:h.target,title:h.title,className:h.className,name:h.name,innerHTML:h.childNodes.length==1&&this.utils.isTextNode(h.firstChild)?h.innerHTML:""};
};
...
},insertSingleLink:function(){var f=Telerik.Web.UI.Editor.DomRange.toDomRange(this.get_editor().getSelection().getRange(true));
f.collapse(true);
...
},shouldReplaceWithEmptyList:function(e){var f=d.isTag;
return !this.isListOn&&!d.isTextNode(e)&&d.isNodeEmpty(e)&&!f(e,"img")&&!f(e,"li")&&!f(e,"td")&&!this.isList(e);
...
(function(a,b,d,c){b.InsertParagraphCommand=function(e,f){this.options=a.extend({tag:"p",retainOriginalNodeName:true,insertParagraphAfterHeading:false,title:"InsertParagraph"},f||{});
b.InsertParagraphCommand.initializeBase(this,[e,this.options]);
this.cursorCssClass="_TELERIK_InsertParagraph";
this.editor=e;
this.container=null;
this.newContainer=null;
this.doc=e.get_document();
};
b.InsertParagraphCommand.prototype={onExecute:function(){try{return this.executeDomCommand();
}catch(e){a("."+this.cursorCssClass,this.editor.get_contentArea()).remove();
}return false;
},executeDomCommand:function(){this.range=this.editor.getSelection().getRange();
this.domRange=domRange=Telerik.Web.UI.Editor.DomRange.toDomRange(this.editor.getSelection().getRange(true));
if(domRange.commonAncestorContainer&&domRange.commonAncestorContainer.nodeName&&/^table|tr$/i.test(domRange.commonAncestorContainer.nodeName)){return false;
}if(this.isOnlyListSelected(domRange)){return this.executeOverList(domRange);
}this.deleteContents();
var f=this.createCursorElement();
f=this.insertNode(f);
if(this.range.selectNodeContents){this.range.selectNodeContents(f);
}else{if(this.range.moveToElementText){this.range.moveToElementText(f);
}}var e=this.wrapInlineElements(f);
if(e.normalize){e.normalize();
}this.clearBRsAroundCursor(f,e);
var g=this.extractFrom(f,e);
g=this.ensureNewLineMode(g);
this.newContainer=g;
this.setCursor(g,f);
return true;
},extractFrom:function(f,e){var g=this.doc.createTextNode("\u00A0");
a(g).insertBefore(f);
var h=new b.DomTreeExtractor(e);
var j=h.extractAfter(g);
var i=e.cloneNode(true);
a(i).insertAfter(e);
while(i.firstChild){i.removeChild(i.firstChild);
}i.appendChild(j);
if(d.getTextContent(e).trim()!=""){a(g).remove();
}return i;
},clearBRsAroundCursor:function(f,e){if(d.isTag(f.previousSibling,"br")){a(f.previousSibling).remove();
}if(d.isTag(f.nextSibling,"br")){a(f.nextSibling).remove();
}},executeOverList:function(f){var e=f.commonAncestorContainer;
var g=doc.createElement(this.options.tag);
g.innerHTML="&nbsp;";
a(g).insertAfter(e);
a(e).remove();
f.setCursorInEmptyNode(g);
return true;
},createCursorElement:function(){var e=this.doc.createElement("span");
e.innerHTML="&nbsp;";
a(e).addClass(this.cursorCssClass);
return e;
},deleteContents:function(){if(this.range.collapsed===true||this.range.text===""){return;
}this.doc.execCommand("delete",false,null);
},insertNode:function(e){if(this.range.insertNode){this.range.insertNode(e);
}else{this.editor.pasteHtml(d.getOuterHtml(e),"",null,null,false);
e=a(this.editor.get_contentArea()).find("."+this.cursorCssClass)[0];
}return e;
},fixListsAroundCursor:function(g,e){var h=this.getParentList(g.startContainer);
var f=this.getParentList(g.endContainer);
if(h){this.removeNonLiElements(h);
}if(f){this.removeNonLiElements(f);
}if(e.parentNode&&(d.isTag(e.parentNode,"ol")||d.isTag(e.parentNode,"ul"))){this.fixListItems(e);
}},regExpWhiteSpace:/^[\s\t\r\n\u200b\u00a0]*$/,ensureNewLineMode:function(e){var f=this.editor.get_newLineMode();
var g="p",h="";
g=f==Telerik.Web.UI.EditorNewLineModes.Div?"div":g;
if(!this.options.retainOriginalNodeName&&!this.container){h=d.getTextContent(e).replace(this.regExpWhiteSpace,"");
if(f==Telerik.Web.UI.EditorNewLineModes.P&&!d.isTag(e,"p")&&h==""){e=d.replaceNode(e,"p");
}else{if(f==Telerik.Web.UI.EditorNewLineModes.Div&&!d.isTag(e,"div")&&h==""){e=d.replaceNode(e,"div");
}}}if(this.options.insertParagraphAfterHeading){h=d.getTextContent(e).replace(this.regExpWhiteSpace,"");
if(/^h[1-6]$/i.test(e.nodeName)&&h==""){e=d.replaceNode(e,g);
}}return e;
},isOnlyListSelected:function(f){if(!f.commonAncestorContainer){return false;
}if(!d.isTag(f.commonAncestorContainer,"ol")&&!d.isTag(f.commonAncestorContainer,"ul")){return false;
}var e=f.commonAncestorContainer;
if(this.isFirstLiSelected(e,f.startContainer)&&this.isLastLiSelected(e,f.endContainer)){return true;
...
},wrapInlineElements:function(f){var g=this.get_window().document;
if(this.container&&a(this.container).find(f).length>0){this._trimElements(this.container,f);
return this.container;
}var e=this.getBlockContainer(f);
if(!d.isBlockElement(e)){a(e).wrap(g.createElement(this.options.tag));
e=e.parentNode;
this.prependPreviousSiblings(e);
this.insertNextSiblings(e);
d.trimWhitespaceTextNodes(e);
}this._trimElements(e,f);
return e;
},_trimElements:function(e,f){this.fixListsAroundCursor(this.domRange,f);
var h=a(e).find("ol,ul");
for(var g=0;
g<h.length;
g++){this.trimElementNeighbours(h[g]);
}if(h.length>0){this.trimElementNeighbours(f);
}if($telerik.isIE&&!$telerik.isIE9Mode){if(f.previousSibling&&d.isTextNode(f.previousSibling)&&this.regExpWhiteSpace.test(f.previousSibling.nodeValue)){a(f.previousSibling).remove();
}if(f.nextSibling&&d.isTextNode(f.nextSibling)&&this.regExpWhiteSpace.test(f.nextSibling.nodeValue)){a(f.nextSibling).remove();
}}},trimElementNeighbours:function(e){d.trimElementNeighbours(e);
},traverseCondition:function(e){return !Telerik.Web.UI.Editor.Utils.isBlockElement(e);
},prependPreviousSiblings:function(e){while(e.previousSibling&&this.traverseCondition(e.previousSibling)){if(d.isTextNodeEmpty(e.previousSibling)){a(e.previousSibling).remove();
}else{a(e).prepend(e.previousSibling);
}}},insertNextSiblings:function(e){while(e.nextSibling&&this.traverseCondition(e.nextSibling)){if(d.isTextNodeEmpty(e.nextSibling)){a(e.nextSibling).remove();
}else{a(e.nextSibling).insertAfter(e.lastChild);
...
},setCursor:function(e,f){var g=this.get_window().document,n=this.range,o;
var m=d.getPreviousElement(f);
var k=d.getNextElement(f);
if((m&&(d.isTag(m,"ol")||d.isTag(m,"ul")))||(k&&(d.isTag(k,"ol")||d.isTag(k,"ul")))){d._setCursorAroundList(f,e,this.editor,n);
}else{if(d.getTextContent(e)==d.getTextContent(f)){this.setCursorInNode(f,e,n);
}else{if(f.nextSibling&&d.isTextNode(f.nextSibling)&&!d.isTextNodeEmpty(f.nextSibling)){if($telerik.isIE){var l=f.parentNode;
a(f).remove();
var h=Telerik.Web.UI.Editor.DomRange.toDomRange(this.editor.getSelection().getRange(true));
var j=d.isTextNode;
var i=a(l).contents().filter(function(){return j(this);
}).get(0);
this.setCursorInTextNode(h,i,0,0);
return;
}else{this.editor.getSelection().moveToElementText(f.nextSibling);
}if($telerik.isSafari&&!$telerik.isChrome){o=f.nextSibling;
a(f).remove();
this.editor.getSelection().moveToElementText(o);
}}else{o=g.createTextNode("");
f.parentNode.insertBefore(o,f);
this.setCursorInTextNode(n,o,0,0);
}}}a(f).remove();
},setCursorInNode:function(f,e,g){d._setCursorInNode(f,e,this.editor,g);
},setCursorInTextNode:function(g,h,f,e){d._setCursorInTextNode(g,h,f,e);
...
}}if(e.previousSibling&&d.isTag(e.previousSibling,"li")&&e.previousSibling.innerHTML==""){a(e).wrap(e.previousSibling);
}else{if(e.nextSibling&&d.isTag(e.nextSibling,"li")&&e.nextSibling.innerHTML==""){a(e).wrap(e.nextSibling);
}}if(e.parentNode.previousSibling&&e.parentNode.previousSibling.innerHTML==""){a(e.parentNode.previousSibling).remove();
}if(e.parentNode.nextSibling&&e.parentNode.nextSibling.innerHTML==""){a(e.parentNode.nextSibling).remove();
}}};
...
var f=a.makeArray(a(g).children());
...
this.AjaxSpellCheck=function(d,f,c){function e(){if(!f.get_ajaxSpellCheck()){var m={editor:f};
var k=document.createElement("div");
var l=$create(Telerik.Web.UI.Editor.AjaxSpellCheck,m,null,null,k);
var n=l.get_element();
var o=$get(f.get_id()+"Top");
if(o){o.appendChild(n);
}f.set_ajaxSpellCheck(l);
}var j=f.get_ajaxSpellCheck();
if(c&&c.value){j.set_language(c.value);
}j.spellCheck();
}if(typeof(Telerik.Web.UI.Editor.AjaxSpellCheck)=="undefined"){function g(j){var k=j.get_responseData();
$telerik.evalScriptCode(k);
e();
}var h=f.get_ajaxSpellCheckScriptReference();
...
this.ModuleManager=function(d,e,c){var h=c.value;
var g=e.get_modulesManager();
if(!g){return false;
}var f=g.getModuleByName(h);
if(f){f.toggleVisibility();
e._updateEditorSize(null,true);
}return false;
};
this.ToggleScreenMode=function(d,e,c){e.toggleScreenMode();
return false;
};
this.InsertRowAbove=this.InsertRowBelow=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableInsertRow(e.getLocalizedString(d),null,d=="InsertRowAbove"?"above":"below",e));
};
this.InsertColumnLeft=this.InsertColumnRight=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableInsertColumn(e.getLocalizedString(d),null,d=="InsertColumnLeft"?"left":"right",e));
};
this.DeleteRow=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableDeleteRow(e.getLocalizedString(d),null,e));
};
this.DeleteColumn=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableDeleteColumn(e.getLocalizedString(d),null,e));
};
this.DeleteCell=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableDeleteCell(e.getLocalizedString(d),null,e));
};
this.MergeColumns=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableMergeColumns(e.getLocalizedString(d),null,e));
};
this.MergeRows=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableMergeRows(e.getLocalizedString(d),null,e));
};
this.SplitCell=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableSplitCell(e.getLocalizedString(d),null,"vertical",e));
};
this.SplitCellHorizontal=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.TableSplitCell(e.getLocalizedString(d),null,"horizontal",e));
};
this.DeleteTable=function(d,e,c){var f=c.value;
if(!f){f=e.getSelectedElement();
}if(f&&"TABLE"!=f.tagName){f=Telerik.Web.UI.Editor.Utils.getElementParentByTag(f,"TABLE");
}if(f){if(!e.get_enableTrackChanges()||e.get_enableTrackChangesOverride()){e.selectElement(f);
e.executeBrowserCommand("Delete");
}else{var g=e.getSelection();
Telerik.Web.UI.Editor.TrackChangesUtils.wrapElementInTrackChangeDelete(g.getParentElement(),e._reviewer);
}}};
this.InsertFormForm=this.InsertFormButton=this.InsertFormCheckbox=this.InsertFormHidden=this.InsertFormImageButton=this.InsertFormPassword=this.InsertFormRadio=this.InsertFormReset=this.InsertFormSubmit=this.InsertFormTextarea=this.InsertFormText=function(d,e,c){var g=d.substring(10);
var f=new Telerik.Web.UI.EditorCommandEventArgs(d,c.get_tool(),g);
e.fire("InsertFormElement",f);
};
this.StripAll=this.StripCss=this.StripFont=this.StripSpan=this.StripWord=function(d,e,c){var g=d.substring(5);
var f=new Telerik.Web.UI.EditorCommandEventArgs(d,c.get_tool(),g.toUpperCase());
e.fire("FormatStripper",f);
};
this.FormatStripper=function(l,n,c){var k=c.value;
var q=n._filtersManager.getFilterByName("ConvertFontToSpanFilter");
if(n.get_mode()==Telerik.Web.UI.EditModes.Design&&k=="SPAN"&&q&&q.get_enabled()){k="FONT";
}var r="";
try{r=n.getSelectionHtml();
}catch(m){}var u=n.getSelection().getBrowserSelection();
var s=u.type?u.type.toLowerCase()=="none":u.isCollapsed;
var o=n.get_contentArea().innerHTML;
if(s||r==""||o==r){var i=Telerik.Web.UI.Editor.Utils.stripFormatting(o,k);
var t=new Telerik.Web.UI.Editor.GenericCommand(n.getLocalizedString(l)+" "+k,n.get_contentWindow(),n);
Telerik.Web.UI.Editor.Utils.setElementInnerHtml(n.get_contentArea(),i);
n.executeCommand(t);
}else{if(r!=null){var h=Telerik.Web.UI.Editor.Utils.stripFormatting(r,k);
var f=true;
if($telerik.isSafari&&k=="ALL"){var v=n.getSelection().getParentElement();
var g=function(p){var e=false;
switch(p){case"I":case"B":case"U":case"SPAN":case"FONT":case"EM":case"STRONG":e=true;
break;
}return e;
};
if(v.innerHTML.trim()==r.trim()&&g(v.tagName)){var d=true;
while(d){var w=v.parentNode;
if(w.childNodes.length==1&&g(w.tagName)){v=w;
}else{d=false;
}}var x=v.parentNode;
var j=n.get_document().createTextNode(h);
x.insertBefore(j,v);
x.removeChild(v);
n.selectElement(j);
f=false;
}}if(f){n.pasteHtml(h,l);
}}}};
this.InsertTable=function(d,e,c){var h=c.value;
if(h){var f=Telerik.Web.UI.Editor.Utils.createTable(h.rows,h.cols);
var g=Telerik.Web.UI.Editor.Utils.getOuterHtml(f);
e.pasteHtml(g,d);
Telerik.Web.UI.Editor.Utils.removeFormattingNodes(e);
}};
this.InsertShiftTab=function(c,d,f){var e=d.getSelectedElement();
if(typeof(e.tagName)=="undefined"){e=e.parentNode;
}var g=Telerik.Web.UI.Editor.Utils.getElementParentByTag(e,"TD");
if(g){Telerik.Web.UI.Editor.Utils.MoveToPreviousCell(g,d);
}else{d.bubbleKeyEventToBrowser();
}return false;
};
this.InsertTabMozilla=function(c,d,f){var e=d.getSelectedElement();
if(typeof(e.tagName)=="undefined"){e=e.parentNode;
}var g=Telerik.Web.UI.Editor.Utils.getElementParentByTag(e,"TD");
if(g){Telerik.Web.UI.Editor.Utils.MoveToNextCell(g,d);
}else{d.bubbleKeyEventToBrowser();
}return false;
};
this.InsertTab=function(c,d,f){var e=d.getSelectedElement();
var g=e.tagName;
if(g=="UL"||g=="OL"||g=="LI"||(e.parentNode&&e.parentNode.tagName=="LI")){d.fire("Indent");
}else{var h=Telerik.Web.UI.Editor.Utils.getElementParentByTag(e,"TD");
if(h){Telerik.Web.UI.Editor.Utils.MoveToNextCell(h,d);
}else{d.pasteHtml(" &nbsp;&nbsp;&nbsp;&nbsp;",c);
}}return false;
};
this.SetToolFocus=function(d,e,f){var c=e.get_toolAdapter();
if(c){c.setFocus();
}return false;
};
this.Undo=this.Redo=function(d,e,c){var f=c?c.value:1;
if(!f){f=1;
}if(d=="Redo"){e.redo(f);
}else{e.undo(f);
}};
this.PastePlainTextWithTable=function(g,i,c){if(!i.get_lockFormatting()){return;
}var o=false;
var l=i.createRestorePoint();
var d=function(p){if(p){if(l){l.select();
l=null;
}var q=p;
q=Telerik.Web.UI.Editor.Utils.replaceNewLineWithBr(q);
if(q){i.pasteHtml(q,g);
}}};
if(i.isIE){var h=Telerik.Web.UI.Editor.Utils.getClipboardAsHtml();
var f=window.clipboardData.getData("Text");
o=j(h);
if(!o){if(f){f=f.replace(/\&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
}d(f);
}else{f=h;
f=k(f);
f=e(f);
f=n(f);
f=m(f);
f=f.replace(new RegExp("(<[^>]+) class=[^ |^>]*([^>]*>)","ig"),"$1 $2");
f=f.replace(/(<[^>]+) style="[^"]*"([^>]*>)/ig,"$1 $2");
f=f.replace(new RegExp("<(SCRIPT)([^>]*)/>","ig"),"");
f=f.replace(new RegExp("<(SCRIPT)([^>]*)>[\\s\\S]*?</(SCRIPT)([^>]*)>","ig"),"");
f=f.replace(/<\s*p.*?>/i,"");
f=f.replace(/<\s*\/\s*p\s*.*?>/i,"");
f=f.replace(/<\s*p.*?>/ig,"<BR>");
f=f.replace(/<\s*\/\s*p\s*.*?>/ig,"");
f=f.replace(/\u00b7/ig,"&bull;");
f=f.replace(/(\S+)=["']?((?:.(?!["']?\s+(?:\S+)=|[>"']))+.)["']?/ig,"");
f=f.replace(/border\s*=\s*(\d)/ig,"");
f=f.replace(/cellSpacing\s*=\s*(\d)/ig,"");
f=f.replace(/cellPadding\s*=\s*(\d)/ig,"");
d(f);
}}function k(p){var q=p;
q=q.replace(/\s*href\s*=\s*('[^']*'|"[^"]*")/ig,"");
var r=new RegExp("<(script|style)[^>]*?>(?:.|\n)*?</(style|script)>","ig");
q=q.replace(r,"");
return q;
}function j(p){var r=document.createElement("DIV");
r.innerHTML=p;
...
u.innerHTML=r;
var q=u.getElementsByTagName("TD");
if(q.length>0){for(var s=0;
s<q.length;
s++){var p=q[s];
p.innerHTML=p.innerHTML.replace(/<\/?p[^>]*>/ig,"");
}t=u.innerHTML;
}u=null;
return t;
}function n(q){var s=q;
var t=/<SPAN[^>]*?>([\s\S]*?)<\/SPAN[^>]*?>/ig;
while(s.match(t)){s=s.replace(t,"$1");
}var p=/<FONT[^>]*?>([\s\S]*?)<\/FONT[^>]*?>/ig;
while(s.match(p)){s=s.replace(p,"$1");
...
Telerik.Web.UI.Editor.Utils.setElementInnerHtml(r,s);
Telerik.Web.UI.Editor.Utils.clearWordAttributesInElement(r,"WORD_ALL");
s=r.innerHTML;
s=s.replace(/\t/g," ");
s=s.replace(/<\/?\w+:[^>]*>/gi,"");
s=s.replace(/<\\?\??xml[^>]>/gi,"");
...
s=Telerik.Web.UI.Editor.Utils.cleanStylesOnStripWordFormatting(s);
s=s.replace(/v:shape="[^"]+"/ig,"");
s=s.replace(/<span[^>]*>\s*<\/span[^>]*>/ig," ");
s=s.replace(/<font[^>]*>\s*<\/font[^>]*>/ig," ");
s=s.replace(/\s+/ig," ");
return s;
}function m(p){var q=p;
q=q.replace(/<\/?font[^>]*>/ig,"");
q=q.replace(/<\/?span[^>]*>/ig,"");
q=q.replace(/<\/?div[^>]*>/ig,"");
q=q.replace(/<\/?a[^>]*>/ig,"");
q=q.replace(/<\s*\/?\s*h([1-6])\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*i\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*b\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*u\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*s\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*em\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*strong\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*strike\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*sup\s*.*?>/ig,"");
q=q.replace(/<\s*\/?\s*sub\s*.*?>/ig,"");
return q;
}};
this.CopyFromEditor=function(e){var g=e.createRestorePoint();
var h=false;
var d=Telerik.Web.UI.Editor.Utils.getClipboardAsHtml();
var c=e.get_clipboardData();
if(c&&c.contentFromEditor&&c.contentHTML==d){h=true;
}else{var f={contentFromEditor:false,contentHTML:d};
e.set_clipboardData(f);
}g.select();
return h;
};
this.PastePlainText=function(f,g,e){var d=function(h){if(h){var i=h.replace(/\&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
i=Telerik.Web.UI.Editor.Utils.replaceNewLineWithBr(i);
if(i){g.pasteHtml(i,f);
}}};
if(g.isIE){d(window.clipboardData.getData("Text"));
}else{var c=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(i,h){d(h.get_content());
});
g.showDialog("CleanPasteTextContent",{},c);
return false;
}};
this.PasteFromWord=this.PasteFromWordNoFontsNoSizes=this.PasteAsHtml=function(f,h,e){var d=function(l,m){var k="";
var n=h.get_stripFormattingOptions();
if(f=="PasteFromWord"){if(n==Telerik.Web.UI.StripFormattingOptions.None||(n&Telerik.Web.UI.StripFormattingOptions.ConvertWordLists)){l=Telerik.Web.UI.Editor.Utils.convertWordLists(l);
}k=Telerik.Web.UI.Editor.Utils.stripFormatting(l,"WORD");
}else{if(f=="PasteFromWordNoFontsNoSizes"){if(n==Telerik.Web.UI.StripFormattingOptions.None||(n&Telerik.Web.UI.StripFormattingOptions.ConvertWordLists)){l=Telerik.Web.UI.Editor.Utils.convertWordLists(l);
}k=Telerik.Web.UI.Editor.Utils.stripFormatting(l,"WORD_ALL");
}else{k=Telerik.Web.UI.Editor.Utils.convertText2Html(l);
}}if(m){m.select();
}if(k){h.pasteHtml(k,f);
}};
if(h.isIE){var j=h.createRestorePoint();
j.useOldRange=true;
var g="";
if(h.get_contentAreaMode()==Telerik.Web.UI.EditorContentAreaMode.Div){var i=h.get_document().selection.createRange();
g=Telerik.Web.UI.Editor.Utils.getClipboardAsHtml(h);
i.select();
}else{g=Telerik.Web.UI.Editor.Utils.getClipboardAsHtml(h);
}d(g,j);
}else{var c=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(l,k){d(k.get_content());
});
h.showDialog("CleanPasteHtmlContent",{dialogTitle:h.getLocalizedString(f)},c);
}};
this.PasteMarkdown=function(e,f,d){var c=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(d,function(j,g){var i=g.get_content();
var h=new Telerik.Web.UI.Editor.Markdown.Converter();
f.pasteHtml(h.makeHtml(i));
});
f.showDialog("PasteMarkdown",{dialogTitle:f.getLocalizedString(e),commandName:"PasteMarkdown"},c);
};
this.PasteHtml=function(e,f,d){var c=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(d,function(i,g){var h=g.get_content();
h=f.applyFiltersForEmbedContent(h);
f.pasteHtml(h);
});
f.showDialog("PasteHtml",{dialogTitle:f.getLocalizedString(e),commandName:"PasteHtml"},c);
};
this.Copy=function(d,g,c){if(null!=c){var h=false;
try{h=g.get_document().queryCommandEnabled(d);
}catch(f){h=false;
}if(h){try{h=g.get_document().execCommand(d,false,null);
}catch(f){h=false;
}}if(!h){alert(g.getLocalizedString("UseCtrlC"));
}}};
this.Cut=this.Paste=function(g,i,f){var p=i.get_enableTrackChanges()||i.get_enableComments();
if(p){var o=null;
if(g=="Cut"){o=i.getToolByName("Cut");
}if(g=="Paste"){o=i.getToolByName("Paste");
}if(o&&!o.get_enabled()){return;
}if(g=="Paste"){i._cachedRange=Telerik.Web.UI.Editor.TrackChangesUtils.getDomRange(i);
if(i.get_lockFormatting()&&!Telerik.Web.UI.Editor.CommandList.CopyFromEditor(i)){var d=i;
var c=f;
window.setTimeout(function(){Telerik.Web.UI.Editor.CommandList.PastePlainTextWithTable("PastePlainTextWithTable",d,c);
},0);
if($telerik.isIE){return;
}}}}if(null!=f||($telerik.isIE&&p&&g=="Cut")){var l=$telerik.isIE7;
var k=l?true:false;
try{if(!l){k=i.get_document().queryCommandEnabled(g);
}}catch(h){k=false;
}if(g=="Paste"&&k&&l){i.get_contentArea().fireEvent("onbeforepaste");
}if(k){var m=new Telerik.Web.UI.Editor.GenericCommand(i.getLocalizedString(g),i.get_contentWindow(),i);
var n=false;
try{n=i.get_document().execCommand(g,false,null);
}catch(h){n=false;
}if(n!=false){i.executeCommand(m);
}else{k=false;
}}if(!k){var j=(g=="Cut"?"UseCtrlX":"UseCtrlV");
alert(i.getLocalizedString(j));
}}else{i._pendingCutPasteCommand=new Telerik.Web.UI.Editor.GenericCommand(i.getLocalizedString(g),i.get_contentWindow(),i);
}};
this.InsertParagraph=function(d,e,f){if($telerik.isIE||$telerik.isFirefox||$telerik.isSafari||$telerik.isOpera11){var c=new Telerik.Web.UI.Editor.InsertParagraphCommand(e);
e.executeCommand(c);
}else{e.executeBrowserCommand(d,true,"");
}return true;
};
this.InsertParagraphSafari=function(c,f,h){f.executeBrowserCommand("InsertParagraph",true,"");
if($telerik.isSafari){var g=f.getSelectedElement();
var d=g;
while(d){if(Telerik.Web.UI.Editor.Utils.isEditorContentArea(d)){d=null;
}else{if(d.tagName=="DIV"){break;
}else{d=d.parentNode;
}}}if(d){var j=d.previousSibling;
if(j){var l=j.tagName;
if(j.nodeType=3||l=="P"||l=="UL"||l=="OL"||(l.charAt(0)=="H"&&!isNaN(parseInt(l.charAt(1))))){var e=f.get_document();
var i=e.createElement("P");
i.innerHTML="<br>";
d.parentNode.insertBefore(i,d);
if(j.nodeType=3){d.innerHTML=d.innerHTML.replace(/^<br>(&nbsp;)*/,"");
Telerik.Web.UI.Editor.Utils.removeNode(d);
}else{d.parentNode.removeChild(d);
}var k=e.createRange();
k.selectNodeContents(i);
f.getSelection().selectRange(k);
}}}}return true;
};
this.FormatBlock=function(d,e,c){e.executeCommand(new Telerik.Web.UI.Editor.FormatBlockCommand(e.getLocalizedString(d),e.get_contentWindow(),c.value,e));
};
this.FormatSets=function(e,f,c){var d=new Telerik.Web.UI.Editor.FormatSetsCommand(f.getLocalizedString(e),f.get_contentWindow(),c.value,f);
d.set_selectedElement(f.getSelectedElement());
f.executeCommand(d);
};
this.FormatPainterApply=function(e,f,c){var d=new Telerik.Web.UI.Editor.FormatPainterCommand(f.getLocalizedString(e),f.get_contentWindow(),null,f);
f.executeCommand(d);
d.set_formatPainterToolSelectedIndex(1);
};
this.FormatPainterCopy=function(e,f,c){var d=new Telerik.Web.UI.Editor.FormatPainterCommand("",f.get_contentWindow(),null,f),g=d.get_formatting(f.getSelectedElement());
d.set_currentFormatting(g);
d.set_formatPainterToolSelectedIndex(1);
};
this.FormatPainterClear=function(e,f,c){var d=new Telerik.Web.UI.Editor.FormatPainterCommand("",f.get_contentWindow(),null,f);
d.set_currentFormatting(null);
d.set_formatPainterToolSelectedIndex(0);
};
this.InsertOrderedList=this.InsertUnorderedList=function(d,e,c){e.setFocus();
e.executeCommand(new Telerik.Web.UI.Editor.InsertListCmd(e,{listType:d=="InsertOrderedList"?"ol":"ul"}));
return true;
e.executeCommand(new Telerik.Web.UI.Editor.InsertListCommand(e.getLocalizedString(d),e.get_contentWindow(),e.get_newLineMode()==Telerik.Web.UI.EditorNewLineModes.Br,d,null,e));
};
this.Indent=function(e,f,c){var d=new Telerik.Web.UI.Editor.IndentCommand(f,{title:e});
f.executeCommand(d);
};
this.Outdent=function(e,f,c){var d=new Telerik.Web.UI.Editor.OutdentCommand(f,{title:e});
f.executeCommand(d);
};
function b(c){return Telerik.Web.UI.Editor[c+"Command"];
}function a(e,d){var c=b(d);
return new c(e,{title:d});
}this.Bold=this.Italic=this.Underline=this.StrikeThrough=function(d,e,f){e.setActive();
e.setFocus();
var c=a(e,d);
e.executeCommand(c);
return true;
};
this.SelectAll=this.Unlink=this.Subscript=this.Superscript=this.AbsolutePosition=function(d,e,g){e.setActive();
var c="SelectAll"!=d;
if(d=="Unlink"&&!e.isIE){var f=e.getSelectedElement();
if(f&&f.tagName=="A"){e.selectElement(f,false);
}}e.executeBrowserCommand(d,c,null,null);
return true;
};
this.JustifyLeft=this.JustifyRight=this.JustifyCenter=this.JustifyNone=this.JustifyFull=function(d,e,f){e.setActive();
var g=e.getLocalizedString(d);
var c=new Telerik.Web.UI.Editor.AlignCommand(e,{title:g,align:d.replace("Justify","").replace("Full","justify").toLowerCase()});
e.executeCommand(c);
return true;
};
this.ForeColor=this.BackColor=this.FontName=this.FontSize=function(d,e,c){var f=new Telerik.Web.UI.Editor.ieFontCommandFix(e);
f.applyFix();
e.executeBrowserCommand(d,true,c.value);
f.cleanUp();
};
this.Zoom=function(d,e,c){var h=c.value;
var f=c.get_tool?c.get_tool():null;
if(h&&f&&f.updateValue){f.updateValue(h);
}e.get_contentArea().style.zoom=h;
if(($telerik.isFirefox||$telerik.isOpera)&&h.indexOf("%")!=-1){h=parseInt(h,10)/100;
}if($telerik.isFirefox){e.get_contentArea().style.MozTransform=h==1?"":"translate("+(e.get_contentArea().offsetWidth*((h-1)/2))+"px, "+(e.get_contentArea().offsetHeight*((h-1)/2))+"px) scale("+h+")";
}if($telerik.isOpera){var g=h==1?"":"translate("+(e.get_contentArea().offsetWidth*((h-1)/2.04))+"px, "+(e.get_contentArea().offsetHeight*((h-1)))+"px) scale("+h+")";
e.get_contentArea().style.OTransform=g;
}return false;
};
this.Print=function(d,e,c){if(e.isIE){e.get_document().execCommand(d,false,null);
}else{if(e.get_contentWindow().print){e.get_contentWindow().print();
}}return false;
};
this.InsertSnippet=function(d,e,c){e.pasteHtml(c.value,d);
};
this.InsertFormElement=function(d,e,c){var j=c.value;
j=j.toLowerCase();
var f=null;
switch(j){case"form":if(e.get_contentAreaMode()==Telerik.Web.UI.EditorContentAreaMode.Div){alert("Error! Form elements are only supported when the content area is set to Iframe!");
c.set_cancel(true);
return false;
}else{f=e.createElement("form","150px","150px");
f.innerHTML="&nbsp;";
}break;
case"textarea":f=e.createElement("textarea");
break;
case"select":f=e.createElement("select","100px","22px");
break;
case"checkbox":case"radio":f=e.createElement("input");
f.setAttribute("type",j);
break;
case"button":case"reset":case"submit":f=e.createElement("input","50px","22px");
f.setAttribute("type",j);
break;
case"hidden":case"password":case"text":f=e.createElement("input","100px","22px");
f.setAttribute("type",j);
break;
}if(f){var h=Telerik.Web.UI.Editor.Utils.getUniqueID();
f.setAttribute("id",h);
var g=Telerik.Web.UI.Editor.Utils.getOuterHtml(f);
e.pasteHtml(g,d);
var i=e.get_document().getElementById(h);
if(i){i.removeAttribute("id");
if(i.setActive){i.setActive();
}return false;
}}};
this.InsertGroupbox=this.InsertDate=this.InsertTime=this.InsertSymbol=this.InsertHorizontalRule=function(d,g,c){var j="";
switch(d){case"InsertSymbol":j=c.value;
break;
case"InsertHorizontalRule":j="<hr>";
break;
case"InsertDate":var e=new Date();
j="&nbsp;"+e.toLocaleDateString();
break;
case"InsertTime":var f=new Date();
j="&nbsp;"+f.toLocaleTimeString();
break;
default:var i=g.getLocalizedString("RadEditorGroupboxTitle","Title");
var h=g.getLocalizedString("RadEditorGroupboxContent","Content...");
j="<fieldset style='WIDTH: 200px; HEIGHT: 76px'> <legend>"+i+"</legend>"+h+" </fieldset> ";
}g.pasteHtml(j,d);
};
this.ImageManager=function(f,g,e){var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(o,k){var n=k.get_value();
var l="";
for(var m=0;
m<n.length;
m++){l+=Telerik.Web.UI.Editor.Utils.getOuterHtml(n[m]);
}g.pasteHtml(l,f);
});
var i=g.get_dialogOpener().get_additionalQueryString();
var j=g.getSelectedElement();
if(j&&j.tagName=="IMG"){var h=j.getAttribute("src",2);
g.get_dialogOpener().set_additionalQueryString(i+"&rndnum="+(new Date()-100)+"&PreselectedItemUrl="+encodeURIComponent(h));
}else{j=null;
}var c=new Telerik.Web.UI.EditorCommandEventArgs(f,null,j);
Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"IMG",g,f);
g.showDialog("ImageManager",c,d);
g.get_dialogOpener().set_additionalQueryString(i);
return false;
};
this.SilverlightManager=this.FlashManager=this.MediaManager=function(f,g,e){var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(m,k){var l=k.get_value();
if(!l){l=k.Result;
}l=g.applyFiltersForEmbedContent(l);
g.pasteHtml(l,f);
});
var c={};
var i=g.get_dialogOpener().get_additionalQueryString();
var j=g.getSelectedElement();
if(j){if($telerik.isIE&&j.object){var h="MediaManager"==f?j.object.FileName:j.object.Movie;
if(h){g.get_dialogOpener().set_additionalQueryString(i+"&rndnum="+(new Date()-100)+"&PreselectedItemUrl="+encodeURIComponent(h));
c.selectedItemUrl=h;
c.selectedObject=j;
}}}g.showDialog(f,c,d);
g.get_dialogOpener().set_additionalQueryString(i);
...
this.TemplateManager=function(e,f,d){var c=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(d,function(i,g){var h=g.get_value();
if(!h){h=g.Result;
}f.pasteHtml(h,e);
});
f.showDialog("TemplateManager",{},c);
return false;
};
this.AboutDialog=function(c,d,e){d.showDialog("AboutDialog");
return false;
};
this.Help=function(c,d,e){d.showDialog("Help");
return false;
};
this.PageProperties=function(f,g,e){var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(i,h){});
var c={};
Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"BODY",g,f);
g.showDialog("PageProperties",c,d);
return false;
};
this.ImageMapDialog=function(f,g,e){var c=Telerik.Web.UI.Editor.CommandList._getImageMapDialogArgument(g);
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(i,h){Telerik.Web.UI.Editor.CommandList._setImageMapProperties(g,h,f);
return false;
});
g.showDialog("ImageMapDialog",c,d);
return false;
};
this._setImageMapProperties=function(f,c,d){if(!c){return;
}var n=f.getSelectedElement();
var g;
if(n&&n.tagName=="IMG"){g=n;
if(c.ImageSrc!=g.src){g.src=c.ImageSrc;
}}else{if(!c.ImageSrc){return;
}f.pasteHtml('<img src="'+c.ImageSrc+'" id = "__tmp__">',d);
g=f.get_document().getElementById("__tmp__");
g.removeAttribute("id");
if(document.all){var m=f.get_document().body.createTextRange();
m.collapse();
m.moveToElementText(g);
m.select();
}}var p=document.createElement("span");
p.innerHTML=c.MapHtml;
var o=p.getElementsByTagName("map");
if(o.length==0){return;
}var j=o[0].innerHTML;
p=null;
if(!j){var e=g.getAttribute("useMap",2);
if(e){var k=Telerik.Web.UI.Editor.CommandList._getImageMapByName(f,e);
if(k){k.parentNode.removeChild(k);
}}g.removeAttribute("useMap");
return;
}var i=null;
var q=g.getAttribute("useMap",2);
if(q){i=Telerik.Web.UI.Editor.CommandList._getImageMapByName(f,q);
}if(i==null){var h=new Date()-100;
var l="rade_img_map_"+h;
i=f.get_document().createElement("map");
i.id=l;
this._setNameAttribute(i,l);
i=f.get_contentArea().appendChild(i);
g.setAttribute("useMap","#"+l);
g.setAttribute("border","0");
}i.innerHTML=j;
};
this._setNameAttribute=function(c,d){c.removeAttribute("name");
c.removeAttribute("NAME");
c.name=null;
c.name=d;
c.NAME=d;
};
this._getImageMapDialogArgument=function(d){var c={};
c.editor=d;
var e=d.getSelectedElement();
...
if($telerik.isIE){var h=d.get_document().body.createTextRange();
h.collapse();
h.moveToElementText(e);
h.select();
}if(e.useMap){var g=e.getAttribute("useMap",2);
var f=Telerik.Web.UI.Editor.CommandList._getImageMapByName(d,g);
if(f!=null){c.ImageMapHTML='<map name = "'+g.substr(1)+'">'+f.innerHTML+"</map>";
}else{c.ImageMapHTML="";
}e.style.width=c.ImageWidth;
e.style.height=c.ImageHeight;
}}return c;
};
this._getImageMapByName=function(c,g){g=g.substr(1);
var d=c.get_document();
var f=d.getElementById(g);
if(f!=null){return f;
}var h=d.getElementsByTagName("map");
for(var e=0;
e<h.length;
e++){if(h[e].getAttribute("name")==g){return h[e];
}}return null;
};
this.get_ImportStyleSheetHTML=function(c,d,g){var f="";
if(g){var e=d.get_document().getElementById(g.id);
if(!e){f=Telerik.Web.UI.Editor.Utils.getOuterHtml(g);
}}return f;
};
this._removeStyleSheet=function(d,h){if(h){var e=d.get_document();
var c=true;
...
f<m;
f++){var k=l[f];
if(k.className==j){c=false;
break;
}}if(c){var g=e.getElementById(j);
if(g){g.parentNode.removeChild(g);
}}}}};
this.TableWizard=function(f,g,e){var c=Telerik.Web.UI.Editor.CommandList._getTableArgument(g,null,true,false);
Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"TABLE",g,f);
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(j,h){var i="";
if(h.styleSheetToImport){i=Telerik.Web.UI.Editor.CommandList.get_ImportStyleSheetHTML(f,g,h.styleSheetToImport);
}g.pasteHtml(Telerik.Web.UI.Editor.Utils.getOuterHtml(h.tableToModify)+i,f);
Telerik.Web.UI.Editor.Utils.removeFormattingNodes(g);
if(h.styleSheetToRemove){Telerik.Web.UI.Editor.CommandList._removeStyleSheet(g,h.styleSheetToRemove);
}});
g.showDialog("TableWizard",c,d);
return false;
};
this.InsertFormSelect=function(f,g,e){var h=g.getSelectedElement();
var i;
if(h&&h.nodeName=="SELECT"){i=h;
}var c=Telerik.Web.UI.Editor.CommandList._getSelectArgument(i);
Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"SELECT",g,f);
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(m,j){if(!$telerik.isIE&&i){var k=Telerik.Web.UI.Editor.DomRange.toDomRange(g.getSelection().getRange());
k.selectNode(i);
k.select();
g.pasteHtml(j.selectToInsert.outerHTML);
return;
}var l=j.selectToInsert;
g.pasteHtml(l.outerHTML);
});
g.showDialog("InsertSelectDialog",c,d);
return false;
};
this.InsertTableLight=this.SetTableProperties=function(f,g,e){var c=Telerik.Web.UI.Editor.CommandList._getTableArgument(g,1,false,true);
if(f=="SetTableProperties"&&!c){alert(g.getLocalizedString("tablewarning"));
return false;
}var h=Telerik.Web.UI.Editor.CommandList._getParentTable(g);
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(k,i){if(g.get_enableTrackChanges()){g.set_fromTableProperties(true);
}g.selectElement(h);
var j="";
if(i.styleSheetToImport){j=Telerik.Web.UI.Editor.CommandList.get_ImportStyleSheetHTML(f,g,i.styleSheetToImport);
}g.pasteHtml(Telerik.Web.UI.Editor.Utils.getOuterHtml(i.tableToModify)+j,f);
if(i.styleSheetToRemove){Telerik.Web.UI.Editor.CommandList._removeStyleSheet(g,i.styleSheetToRemove);
}});
if(g.get_document().selection&&g.get_document().selection.empty&&f!="InsertTableLight"){g.get_document().selection.empty();
}if(f=="SetTableProperties"){Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"TABLE",g,f);
g.showDialog("TableWizard",c,d);
...
this.SetCellProperties=function(f,g,e){var c=Telerik.Web.UI.Editor.CommandList._getTableArgument(g,2,false,true);
var h=$telerik.isFirefox&&g.get_contentWindow().getSelection().rangeCount>1;
if(!c||h){alert(g.getLocalizedString("cellwarning"));
return false;
}Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"TABLE",g,f);
var i=Telerik.Web.UI.Editor.CommandList._getParentTable(g);
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(l,j){g.selectElement(i);
var k="";
if(j.styleSheetToImport){k=Telerik.Web.UI.Editor.CommandList.get_ImportStyleSheetHTML(f,g,j.styleSheetToImport);
}g.pasteHtml(Telerik.Web.UI.Editor.Utils.getOuterHtml(j.tableToModify)+k,f);
if(j.styleSheetToRemove){Telerik.Web.UI.Editor.CommandList._removeStyleSheet(g,j.styleSheetToRemove);
}});
g.showDialog("TableWizard",c,d);
return false;
};
this.FindAndReplace=function(f,g,e){var c={};
c.editor=g;
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(i,h){});
g.showDialog("FindAndReplace",c,d);
return false;
};
this.StyleBuilder=function(f,g,e){var l=null;
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(q,m){var o=Telerik.Web.UI.Editor.Utils.getOuterHtml(m.get_value());
o=o.replace(/(url\s*\(\s*)&quot;(.*)&quot;\s*\)/ig,"$1$2)");
if(l){var p="";
var r=new RegExp("style=(\"|')(.+?)\\1","gi");
o.replace(r,function(u,s,t){p=t;
});
if(p){var n=new Telerik.Web.UI.Editor.AttributeCommand(f+"set Style Attribute",g.get_contentWindow(),l,"style",p,g);
g.executeCommand(n);
}}else{g.pasteHtml(o,f);
}});
var j=g.getSelection();
var i=j.getParentElement();
if(!$telerik.isIE||!j.isControl()){var k=j.getText();
if(k||(i&&i.tagName=="BODY")){var h=g.createElement("span");
Telerik.Web.UI.Editor.Utils.setElementInnerHtml(h,k);
i=h;
}else{l=i;
g.selectElement(i,false);
}}if(!i){i=Telerik.Web.UI.Editor.Utils.getContentAreaFirstChild(g);
g.selectElement(i,false);
}var c=new Telerik.Web.UI.EditorCommandEventArgs("StyleBuilder",null,(i?i.cloneNode(true):null));
Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"*",g,f);
c.fontNames=g.get_fontNames();
g.showDialog("StyleBuilder",c,d);
return false;
};
this.XhtmlValidator=function(f,g,e){var c=g;
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(i,h){});
g.showDialog("XhtmlValidator",c,d);
return false;
};
this.TrackChangesDialog=function(f,g,e){var c=g;
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(i,h){});
g.showDialog("TrackChangesDialog",c,d);
};
this.InsertCustomLink=function(d,e,c){var f=c.value;
var g=$telerik.$(String.format("<a href='{1}' title='{3}' target='{2}'>{0}</a>",f.innerHTML,f.href,f.target,f.title))[0];
if($telerik.isIE&&e.get_html()=="<P>&nbsp;</P>"){e.set_html("");
}var h=Telerik.Web.UI.Editor.CommandList._getParentLink(e);
if(h){g=h.cloneNode(true);
if(f.href){g.setAttribute("href",f.href);
}if(f.target){g.setAttribute("target",f.target);
}if(f.title){g.setAttribute("title",f.title);
}if(!g.innerHTML){g.innerHTML=f.innerHTML;
}}e.pasteHyperLink(g,d);
};
this._getDialogArguments=function(c,f,e,d){if(!e){c.Colors=[];
c.CssClasses=[];
c.CellCssClasses=[];
c.tableLayoutCssFile="";
c.editor=null;
return;
}c.Colors=e.get_colors();
if("TD"==f||"TH"==f||"TABLE"==f){c.CssClasses=e.getCssArray("TABLE");
c.CellCssClasses=e.getCssArray("TD");
c.tableLayoutCssFile=e.get_tableLayoutCssFile();
}else{if("A"==f||"BODY"==f||"IMG"==f){c.CssClasses=e.getCssArray(f);
}else{if("SELECT"==f){c.CssClasses=e.getCssArray(f);
}}}c.editor=e;
};
this.DocumentManager=this.SetLinkProperties=this.LinkManager=function(f,h,e){var g=f=="DocumentManager"?"DocumentManager":"LinkManager";
var c=Telerik.Web.UI.Editor.CommandList._getLinkArgument(h);
Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"A",h,f);
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(l,i){var k=i.get_value()||i.Result||i.realLink;
var j=new Telerik.Web.UI.Editor.InsertLinkCommand(f,h.get_contentWindow(),k,h);
h.executeCommand(j);
});
h.showDialog(g,c,d);
return false;
};
this.InsertLink=function(f,h,e){var g="InsertLink";
var c=Telerik.Web.UI.Editor.CommandList._getLinkArgument(h);
c._name=c._commandName="InsertLink";
Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"A",h,f);
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(l,i){var k=i.get_value();
var j=new Telerik.Web.UI.Editor.InsertLinkCommand(f,h.get_contentWindow(),k,h);
h.executeCommand(j);
});
h.showDialog(g,c,d);
...
this.InsertImage=this.SetImageProperties=function(f,h,e){var g=h.getSelectedElement();
if(g&&g.nodeName!="IMG"){g=null;
}var c=new Telerik.Web.UI.EditorCommandEventArgs(f,null,g);
Telerik.Web.UI.Editor.CommandList._getDialogArguments(c,"IMG",h,f);
c.Element=g;
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(l,i){var k=i.get_value?i.get_value():i.Result;
if(!k){return;
}if(g&&g.parentNode){var j=new Telerik.Web.UI.Editor.GenericCommand(h.getLocalizedString(f),h.get_contentWindow(),h);
g.parentNode.replaceChild(k,g);
h.executeCommand(j);
}else{h.pasteHtml(Telerik.Web.UI.Editor.Utils.getOuterHtml(k),f);
}});
if(f=="SetImageProperties"){h.showDialog("ImageProperties",c,d);
}else{h.showDialog("InsertImage",c,d);
}return false;
};
this.InsertExternalVideo=function(e,f,d){var c=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(d,function(h,g){f.pasteHtml(g.get_value(),e);
});
f.showDialog("InsertExternalVideo",null,c);
};
this.FormatCodeBlock=function(f,g,e){var c=new Telerik.Web.UI.EditorCommandEventArgs("FormatCodeBlock",null,g.getSelectionHtml());
c.dirtyCode=g.getSelectionHtml();
if(g.get_document().selection&&g.get_document().selection){var h=g.get_document().selection.type.toLowerCase();
if(h=="control"||h=="none"){c.set_value("");
}}var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(j,i){g.pasteHtml(i.get_value?i.get_value():i.get_code(),f);
});
g.showDialog("FormatCodeBlock",c,d);
return false;
};
this._getDocumentAnchors=function(e){var d=e.getElementsByTagName("A");
var c=new Array();
for(var f=0;
f<d.length;
f++){if(d[f].name){c[c.length]=d[f];
}}return c;
};
this._getParentLink=function(d){var h=d.getSelectedElement();
var g=Telerik.Web.UI.Editor.Utils.getElementParentByTag(h,"A");
if(g){d.selectElement(g);
}else{var f="";
var i=(h&&h.tagName)?h.tagName:"";
if(i=="IMG"){f=Telerik.Web.UI.Editor.Utils.getOuterHtml(h);
}if(!f){f=d.getSelectionHtml();
}if($telerik.isIE){var j=h.tagName;
if(j=="FONT"||j=="SPAN"){f=f.replace(/^<(FONT|SPAN)[^>]*>/gi,"");
f=f.replace(/<\/(FONT|SPAN)>$/gi,"");
}}g=d.get_document().createElement("a");
try{Telerik.Web.UI.Editor.Utils.setElementInnerHtml(g,f);
}catch(c){g.innerHTML=String.format("<span>{0}</span>",Telerik.Web.UI.Editor.Utils.stripFormatting(f,"ALL"));
}}return g;
};
this._getLinkArgument=function(d,h){var g=Telerik.Web.UI.Editor.CommandList._getParentLink(d);
var f=g.innerHTML;
var e=!f?false:f.match(/</);
if($telerik.isIE&&e){var i=f.trim().toLowerCase();
if(i.match(/<p[^>]*>(&nbsp;|\s*)<\/p>/)){g.innerHTML=" ";
e=false;
}}var c=new Telerik.Web.UI.EditorCommandEventArgs("LinkManager",null,g.cloneNode(true));
c.selectedTabIndex=h?h:0;
c.showText=!e;
c.documentAnchors=Telerik.Web.UI.Editor.CommandList._getDocumentAnchors(d.get_document());
c.realLink=c.get_value();
return c;
};
...
this._getTableArgument=function(c,i,d,h){var g;
var f=null;
if(d){g=Telerik.Web.UI.Editor.Utils.createTable(2,2,c.get_document());
}else{g=Telerik.Web.UI.Editor.CommandList._getParentTable(c);
}if(!d&&!g){return null;
}if(h){f=Telerik.Web.UI.Editor.CommandList._getParentCell(c);
}var j=g.cloneNode(true);
var e=g;
return{selectedTabIndex:i?i:0,cellToModify:f,tableToModify:j,originalTable:e,tableDocument:j.ownerDocument||c.get_document()};
};
this._getSelectArgument=function(c){return{selectToModify:c};
};
this.EnterParagraphMozilla=function(d,e,c){var f=new Telerik.Web.UI.Editor.EnterNewLineCommand("Enter",e.get_contentWindow(),false,e);
return f.enterBlockElement();
};
this.EnterMozilla=function(d,e,c){if(e.get_mode()==Telerik.Web.UI.EditModes.Preview){return false;
}var f=e.getSelectedElement();
if(f!=null&&f.tagName!="P"){e.bubbleKeyEventToBrowser();
return true;
}else{e.pasteHtml("<br />");
}};
this.EnterNewLine=function(e,f,c){var d=new Telerik.Web.UI.Editor.EnterNewLineCommand("Enter",f.get_contentWindow(),false,f);
if($telerik.isSafari&&f&&f.get_newLineMode()!=Telerik.Web.UI.EditorNewLineModes.Br){d.set_canUnexecute(false);
}f.executeCommand(d);
};
this.Enter=function(d,e,c){if(e.get_mode()==Telerik.Web.UI.EditModes.Preview){return false;
}var k=e.getSelectedElement();
if("LI"==k.tagName||Telerik.Web.UI.Editor.Utils.getElementParentByTag(k,"LI")!=null){e.bubbleKeyEventToBrowser();
return false;
}if(e.get_enableTrackChanges()){if(k.getAttribute("Name")!=null){if(k.getAttribute("Name").indexOf("_wp")==0){e.bubbleKeyEventToBrowser();
return false;
}}}try{var h=e.getLocalizedString("Enter");
var g=new Telerik.Web.UI.Editor.GenericCommand(h,e.get_contentWindow(),e);
var i=e.get_document().selection.createRange();
if(i.pasteHTML){var l=k.tagName;
if(l.charAt(0)=="H"&&parseInt(l.charAt(1))>0){var j=i.duplicate();
j.moveToElementText(k);
e.bubbleKeyEventToBrowser();
return false;
}else{if(l.toLowerCase()=="textarea"){e.bubbleKeyEventToBrowser();
return false;
}}i.pasteHTML("<br>");
i.select();
...
this.ShiftEnter=function(d,f,c){var j=f.getSelectedElement();
var e=f.get_document();
if(e.selection&&("LI"==j.tagName||Telerik.Web.UI.Editor.Utils.getElementParentByTag(j,"LI")!=null)){var h=f.getLocalizedString("Enter");
var g=new Telerik.Web.UI.Editor.GenericCommand(h,f.get_contentWindow(),f);
...
this.ShiftEnterSafari=function(d,f,c){var h=f.getSelectedElement();
if("LI"==h.tagName||Telerik.Web.UI.Editor.Utils.getElementParentByTag(h,"LI")!=null){var g=new Telerik.Web.UI.Editor.GenericCommand("Enter",f.get_contentWindow(),f);
f.pasteHtml("<br />","",null,null,false);
f.executeCommand(g);
}else{var e=Telerik.Web.UI.Editor.EnterNewLineCommand.insertBrAfterHeading;
Telerik.Web.UI.Editor.EnterNewLineCommand.insertBrAfterHeading=true;
f.executeCommand(new Telerik.Web.UI.Editor.EnterNewLineCommand(f.getLocalizedString(d),f.get_contentWindow(),false,f,Telerik.Web.UI.EditorNewLineModes.Br));
Telerik.Web.UI.Editor.EnterNewLineCommand.insertBrAfterHeading=e;
}return false;
};
this.EnterSafari=function(d,e,c){var f=e.getSelectedElement();
if("LI"==f.tagName||Telerik.Web.UI.Editor.Utils.getElementParentByTag(f,"LI")!=null){e.bubbleKeyEventToBrowser();
return false;
}e.executeBrowserCommand("InsertLineBreak",true,"");
return false;
};
this.ToggleTableBorder=function(d,e,c){e.toggleEnhancedEdit();
return false;
};
this.ConvertToUpper=this.ConvertToLower=function(e,g,c){var u,k,f=g.get_document();
if($telerik.isIE&&f.selection){if(f.selection.type.toLowerCase()=="control"){return;
}var p=f.selection.createRange();
u=p.duplicate();
k=p.duplicate();
u.collapse();
k.collapse(false);
}var r=Telerik.Web.UI.Editor.CommandList._markEditorSelection(g);
var j=r.markedElements;
var n=r.newElements;
var l=new Telerik.Web.UI.Editor.GenericCommand(g.getLocalizedString(e),g.get_contentWindow(),g);
var m;
for(m=0;
m<j.length;
m++){d(j[m]);
...
m<n.length;
m++){var h=n[m];
if($telerik.isIE&&h.removeNode){h.removeNode(false);
}else{var q=document.createRange();
q.selectNodeContents(h);
h.parentNode.replaceChild(q.extractContents(),h);
}}if($telerik.isIE&&f.selection){var o=f.selection.createRange();
o.setEndPoint("StartToStart",u);
o.setEndPoint("EndToEnd",k);
o.select();
}else{var t=g.get_contentWindow().getSelection();
var s=t.getRangeAt(0);
s.collapse(true);
}g.executeCommand(l);
function d(w){var v=w.childNodes;
for(var x=0;
x<v.length;
x++){if(v[x].nodeType==3){v[x].nodeValue=("ConvertToLower"==e)?v[x].nodeValue.toLowerCase():v[x].nodeValue.toUpperCase();
}else{if(v[x].nodeType==1&&v[x].tagName.toUpperCase()!="FONT"){d(v[x]);
}}}}};
this.ApplyClass=function(e,g,c){var t=true;
var n=c.get_value();
var f=g.get_contentWindow();
var h=g.get_document();
var l=new Telerik.Web.UI.Editor.GenericCommand(g.getLocalizedString(e)+' ["'+n+'"]',g.get_contentWindow(),g);
var q=g.getSelectedElement();
var r=(q)?q.tagName:"";
var s=g.getSelection().getHtml();
var d=(!n&&!s)||s.match(/<(font|span)[^>]*><\/(font|span)>/ig)||r=="UL"||r=="IMG";
var o=(d)?q:Telerik.Web.UI.Editor.Utils.getFullySelectedElement(f);
if(o&&!Telerik.Web.UI.Editor.Utils.isEditorContentArea(o)){Telerik.Web.UI.Editor.Utils.setElementClassName(o,n);
t=false;
}else{if(!n){h.execCommand("RemoveFormat",false,null);
t=false;
}}if(t){var p=Telerik.Web.UI.Editor.CommandList._markEditorSelection(g);
var k=p.markedElements;
if(k.length>0){for(var m=0;
m<k.length;
m++){var j=k[m];
if(j.className&&j.className!="Apple-style-span"){continue;
}else{j.className=n;
}}if(!$telerik.isIE){Telerik.Web.UI.Editor.Utils.addElementsToSelection(g.get_contentWindow(),k);
}}else{Telerik.Web.UI.Editor.CommandList._completeEditorSelection(g,"class='"+n+"'");
}}if(c.addToUndoRedo!=false){g.executeCommand(l);
}};
this.RealFontSize=function(d,e,c){var p=c.get_value();
var j=new Telerik.Web.UI.Editor.GenericCommand(e.getLocalizedString(d)+' ["'+p+'"]',e.get_contentWindow(),e);
var n=Telerik.Web.UI.Editor.CommandList._markEditorSelection(e);
...
k++){var f=h[k];
var l=false;
if($telerik.isSafari){var m=f.parentNode;
var o=m.tagName;
if((o=="FONT"||o=="SPAN")&&m.childNodes.length==1){m.style.fontSize=p;
m.removeAttribute("size");
Telerik.Web.UI.Editor.Utils.removeNode(f);
l=true;
g.push(m);
e.selectElement(m);
}}if(!l){f.style.fontSize=p;
f.removeAttribute("size");
g.push(f);
}}if(!$telerik.isIE){Telerik.Web.UI.Editor.Utils.addElementsToSelection(e.get_contentWindow(),g);
}}else{Telerik.Web.UI.Editor.CommandList._completeEditorSelection(e,"style='font-size:"+p+"'");
}e.executeCommand(j);
Telerik.Web.UI.Editor.Utils.removeInvalidElementsInLists(e.get_document());
};
this._completeEditorSelection=function(d,g){if(!g){g="";
}var c=d.get_contentWindow();
var e=d.get_document();
var j=d.getSelection().getParentElement();
var k=j?j.tagName:"";
if($telerik.isIE&&!d.getSelectionHtml()&&(k=="FONT"||k=="SPAN")){e.execCommand("RemoveFormat",null,false);
}var f=d.getSelection();
f.pasteHtml("<font "+g+" id='radERealFont'>&#x200b;</font>");
var h=d.get_document().getElementById("radERealFont");
h.removeAttribute("id");
if($telerik.isIE){d.selectElement(h);
d.getSelection().collapse();
h.innerHTML="";
}else{if(c.getSelection){var i=c.getSelection();
var l=d.getSelection().getRange();
i.removeAllRanges();
if(!$telerik.isSafari){h.innerHTML="";
}l.selectNodeContents(h);
if(!$telerik.isSafari){l.collapse(false);
}i.addRange(l);
}}};
this._markEditorSelection=function(h){if(h.get_html()==""){return{markedElements:[],newElements:[],oldElements:[]};
}var g="AZBY";
var f="_cm";
var u=[];
var v=[];
var w=[];
var x=[];
var r=$telerik.isSafari2||$telerik.isSafari3;
var p=r?"span":"font";
var k=h.get_contentArea();
if(r){var z=new Telerik.Web.UI.Editor.OptimizeSpans();
z.getHtmlContent(k);
}var l=h.getSelection();
if(l.isControl()){var e=l.getParentElement();
var o=h.get_document().createElement(p);
o.appendChild(e.cloneNode(true));
e.parentNode.replaceChild(o,e);
return{markedElements:[o],newElements:[o]};
}t();
if(!$telerik.isIE){k.ownerDocument.execCommand("UseCSS",false,true);
k.ownerDocument.execCommand("styleWithCSS",false,false);
}k.ownerDocument.execCommand("FontName",false,g);
var m=k.getElementsByTagName(p);
for(var q=0;
q<m.length;
q++){var n=m[q];
if(n.getAttribute("face")==g||n.style.fontFamily==g){n.removeAttribute("face");
if(n.style.fontFamily==g){n.style.fontFamily="";
if(n.className=="Apple-style-span"){n.className="";
}}v.push(n);
var d=n.getElementsByTagName(p);
for(var s=0;
s<d.length;
s++){var c=d[s];
if(c.getAttribute("face")!=g){v.push(c);
}}if(!n.getAttribute(f)){w.push(n);
}else{x.push(n);
}}n.removeAttribute(f);
}y();
if(!$telerik.isIE){k.ownerDocument.execCommand("UseCSS",false,false);
k.ownerDocument.execCommand("styleWithCSS",false,true);
}function t(){var j=h.get_contentArea().getElementsByTagName(p);
for(var B=0;
B<j.length;
B++){var A=j[B];
if(j[B].face){A.setAttribute("_face",A.face);
u.push(A);
}else{if(r){A.setAttribute("_face",A.style.fontFamily);
u.push(A);
}}A.setAttribute(f,1);
}}function y(){for(var j=0;
j<u.length;
j++){if(r){u[j].style.fontFamily=u[j].getAttribute("_face");
}else{u[j].face=u[j].getAttribute("_face");
}u[j].removeAttribute("_face");
}u=[];
}return{markedElements:v,newElements:w,oldElements:x};
};
this.CSDialog=function(f,g,e){var c=g;
var d=Telerik.Web.UI.Editor.CommandList.getCallbackFunction(e,function(i,h){});
g.showDialog("CSDialog",c,d);
return false;
};
})();
...
a.registerEnum(b,"EditorFilters",{None:0,RemoveScripts:1,MakeUrlsAbsolute:2,FixUlBoldItalic:4,FixEnclosingP:8,IECleanAnchors:16,MozEmStrong:32,ConvertFontToSpan:64,ConvertToXhtml:128,IndentHTMLContent:256,EncodeScripts:512,OptimizeSpans:1024,ConvertCharactersToEntities:2048,PdfExportFilter:4096,ConvertInlineStylesToAttributes:8192,ConvertTags:16384,DefaultFilters:53213});
...
}b=b.replace(/<BR\s?\/?>\s*<\/(H1|H2|H3|H4|H5|H6|LI|P|DIV)/ig,function(e,d,f,c){if(!c.substr(0,f).match(/<BR\s?\/?>\s*$/gi)){e="</"+d;
}return e;
});
b=b.replace(/<(H1|H2|H3|H4|H5|H6|LI|P|DIV)([^>]*)?><BR\s?\/?>/ig,"<$1 $2>");
...
Telerik.Web.UI.Editor.ConvertTagsFilter=function(){Telerik.Web.UI.Editor.ConvertTagsFilter.initializeBase(this);
this.IsDom=false;
this.Enabled=true;
this.Name="ConvertTagsFilter";
this.Description="This filter converts one type of tags to another type.";
};
Telerik.Web.UI.Editor.ConvertTagsFilter.prototype={tags:[{source:"b",target:"strong"},{source:"i",target:"em"},{source:"strike",target:"del"}],getHtmlContent:function(a){return this.convertTags(a);
},getDesignContent:function(a){return this.convertTags(a);
},convertTags:function(a){var c=a,d=this.tags;
for(var b=0;
b<d.length;
b++){c=this._replaceTag(c,d[b]);
}return c;
},_replaceTag:function(a,b){return a.replace(new RegExp("<"+b.source+"(\\s([^>])*?)?>","ig"),"<"+b.target+"$1>").replace(new RegExp("</"+b.source+"(\\s([^>])*?)?>","ig"),"</"+b.target+"$1>");
}};
Telerik.Web.UI.Editor.ConvertTagsFilter.registerClass("Telerik.Web.UI.Editor.ConvertTagsFilter",Telerik.Web.UI.Editor.Filter);
...
if($telerik.isFirefox){this._fixFontFamilyValues(i,e);
...
var h,d,g;
var i=b.getElementsByTagName("SPAN");
while(i.length>0){h=i[0];
g=h.parentNode;
d=e.cloneNode(false);
...
}if(h.style.fontFamily){d.face=h.style.fontFamily;
if($telerik.isFirefox){var f=this._getFontFamilyValue(h.attributes.style.nodeValue);
d.face=f?f:d.face;
}this._removeElementStyleAttribute(d,"fontFamily");
}if(h.style.fontSize){var a=null;
if(-1!=h.style.fontSize.indexOf("px")){a=this._fontSizesRevPx[parseInt(h.style.fontSize)];
}if(typeof(a)!="undefined"&&null!=a){d.size=a+1;
this._removeElementStyleAttribute(d,"fontSize");
}}if(h.style.color){d.color=Telerik.Web.UI.Editor.Utils._rgbToHex(h.style.color);
...
}}},_getFontFamilyValue:function(c){var b,a;
if(b=c.match(/(font-family:\s*)([^;]+);/i)){a=b[2];
}return a;
},_fixFontFamilyValues:function(b,a){var c=b.attributes.style.nodeValue;
c=c.replace(b.style.fontFamily,a.face.replace(/\s+/g," "));
b.attributes.style.nodeValue=c;
}};
...
}this._setContentToEmptyWrappers(a);
...
return b;
},_setContentToEmptyWrappers:function(a){$telerik.$("p,div",a).each(function(c,b){if(b.innerHTML==""||(b.nodeName=="DIV"&&b.childNodes.length==1&&b.firstChild.nodeName=="BR")){$telerik.$(b).html("&nbsp;");
}});
...
}else{var e=this._getElementCssText(c);
...
}try{var o=this._getNodeAttributes(n);
var k=o.length;
for(var e=0;
e<k;
...
break;
}},_getElementCssText:function(d){var a=d.style.cssText;
if(!a){return"";
}if(!$telerik.isFirefox){return a;
}var h=/(font-family:\s*)([^;]+);/i;
var i;
var e=d.attributes.style;
if(e&&e.nodeValue){i=e.nodeValue;
}if(i&&h.test(i)&&h.test(a)){var c,b;
if(c=a.match(h)){b=c[2];
}var g,f;
if(g=i.match(h)){f=g[2];
}if(f&&b&&f!==b){f=f.replace(/[\s]+/g," ");
a=a.replace(b,f);
}}return a;
},_getNodeAttributes:function(a){var b=a.attributes;
if(!b){return b;
}if($telerik.isFirefox){this.reverseNodeAttributes(a);
}return a.attributes;
},reverseNodeAttributes:function(b){var c=[];
for(var a=0;
a<b.attributes.length;
a++){c.push({attrName:b.attributes[a].nodeName,attrValue:b.attributes[a].nodeValue});
}for(var a=0;
a<c.length;
a++){b.removeAttribute(c[a].attrName);
}for(var a=c.length-1;
a>=0;
a--){b.setAttribute(c[a].attrName,c[a].attrValue);
}}};
...
Telerik.Web.UI.Editor.CleanResizableHandles=function(){Telerik.Web.UI.Editor.CleanResizableHandles.initializeBase(this);
this.set_isDom(true);
this.set_enabled(true);
this.set_name("CleanResizableHandles");
this.set_description("Removes the elements of the Resizable's handles");
};
Telerik.Web.UI.Editor.CleanResizableHandles.prototype={getHtmlContent:function(a){$telerik.$(".rrHandle[rel='rrWidget']",a).remove();
return a;
}};
Telerik.Web.UI.Editor.CleanResizableHandles.registerClass("Telerik.Web.UI.Editor.CleanResizableHandles",Telerik.Web.UI.Editor.Filter);
...
if($telerik.isIE&&($telerik.isIE10||!$telerik.isIE10Mode)){return this._executeIE(b,a);
...
var c=false;
if(($telerik.isFirefox||($telerik.isIE10Mode&&!$telerik.isIE10))&&k.nodeType!=1){var e=k.childNodes;
var m=e.length;
for(var j=0;
j<m;
...
if(($telerik.isFirefox||($telerik.isIE10Mode&&!$telerik.isIE10))&&c){var w=document.createTextNode(u);
...
q.setStartAfter(n);
}else{if(a){q.setStartBefore(a);
q.setEndBefore(a);
}else{if(k.nodeType==11){n=f.childNodes[f.childNodes.length-1];
if(Telerik.Web.UI.Editor.Utils.isTextNodeEmpty(n)){q.setStartBefore(n);
q.setEndBefore(n);
}else{q.setStartAfter(n);
q.setStartAfter(n);
}}else{q.setStartAfter(k);
q.setEndAfter(k);
...
},getRange:function(c){var g=this._window;
if(!g){return null;
}var a=g.document,b=!!a.selection,f=g.getSelection&&(!b||c);
if(f){var e=g.getSelection();
if(!e||e.rangeCount<1){return null;
}var d=null;
if(e.getRangeAt){d=e.getRangeAt(0);
}else{d=a.createRange();
d.setStart(e.anchorNode,e.anchorOffset);
d.setEnd(e.focusNode,e.focusOffset);
}return d;
}else{if(b){return a.selection.createRange();
...
(function(a,b,d,c){b.EmptyWhitespaceCharTracker=function(e){this.editor=e;
this.nodes=[];
this.cleanUpDelegate=Function.createDelegate(this,this.cleanUpWhitespace);
};
b.EmptyWhitespaceCharTracker.prototype={startTracking:function(){var e=this.editor.get_contentArea();
this.isTracking=true;
this.editor.add_submit(this.cleanUpDelegate);
$telerik.addHandler(e,"keypress",this.cleanUpDelegate);
$telerik.addExternalHandler(e,"mousedown",this.cleanUpDelegate);
$telerik.addExternalHandler(e,"touchstart",this.cleanUpDelegate);
},stopTracking:function(){this.detachEvents();
this.isTracking=false;
},trackNode:function(e){this.nodes.push(e);
},cleanUpWhitespace:function(f){if(!this.shouldCleanUp(f)){return;
}var g=this.nodes;
while(g.length){node=g.pop();
try{node.nodeValue=node.nodeValue.replace("?","");
}catch(f){}}},shouldCleanUp:function(f){return f.type!="keypress"||!$telerik.isChrome||(f.keyCode>32&&f.keyCode<40);
},reset:function(){this.nodes=[];
},detachEvents:function(){var e=this.editor.get_contentArea();
this.editor.remove_submit(this.cleanUpDelegate);
$telerik.removeHandler(e,"keypress",this.cleanUpDelegate);
$telerik.removeExternalHandler(e,"mousedown",this.cleanUpDelegate);
$telerik.removeExternalHandler(e,"touchstart",this.cleanUpDelegate);
},dispose:function(){if(!this.isTracking){return;
}this.stopTracking();
this.cleanUpDelegate=null;
this.editor=null;
}};
})($telerik.$,Telerik.Web.UI.Editor,Telerik.Web.UI.Editor.Utils);
Type.registerNamespace("Telerik.Web.UI");
...
},_isEditorRightToLeft:function(){var c=this._isRightToLeft;
if(c==null){var d=this.get_element();
if(d){c=this._isRightToLeft=$telerik.isRightToLeft(d);
}}return c;
...
if(d){this._fixMoreSizeIssues();
}h=$get(this.get_id()+"_BottomTable");
...
this.whitespaceTracker=new b.Editor.EmptyWhitespaceCharTracker(this);
...
e.style.display="";
g._updateEditorSize(g._originalHeight);
},0);
}else{this._modulesManager.initialize();
this._updateEditorSize(this._originalHeight);
}}if(this.get_autoResizeHeight()&&d){this._makeAutoResizeHeight();
...
if(this._isEditorRightToLeft()){var c=this.get_element();
Sys.UI.DomElement.addCssClass(c,"reRtl");
...
this.add_modeChange(a.proxy(function(d,c){if(d.get_mode()==2&&this._elementsResizableWidget){this._disposeElementResizableWidget();
}},this));
this.add_submit(a.proxy(function(){if(this._elementsResizableWidget){this._disposeElementResizableWidget();
}},this));
},_elementResizingSelectionChangeHandler:function(){var d=this._getElementForResizing(this.getSelectedElement());
if(d){if(this._elementsResizableWidget&&this._elementsResizableWidget._element!=d){this._disposeElementResizableWidget();
}if(!this._elementsResizableWidget){this._elementsResizableWidget=this._getResizable(d);
var c=new Telerik.Web.UI.Editor.GenericCommand("Resize "+d.tagName.toLowerCase(),this.get_contentWindow(),this);
var e=this;
this._elementsResizableWidget.add_resizeStart(function(g,f){e.get_contentArea().removeAttribute("contenteditable");
});
this._elementsResizableWidget.add_resizeEnd(function(g,f){e.get_contentArea().setAttribute("contenteditable","true");
e.executeCommand(c);
e._cleanHandlesFromRestorePiont(c);
e._reInitResizable(g);
});
this._markHandlesForContentFilter();
}}else{if(this._elementsResizableWidget){this._disposeElementResizableWidget();
}}},_getCleanResizableHandlesFilter:function(){var c=this.get_filtersManager()?this.get_filtersManager().getFilterByName("CleanResizableHandles"):null;
if(!c){c=new Telerik.Web.UI.Editor.CleanResizableHandles();
}return c;
},_cleanHandlesFromRestorePiont:function(c){if(!c.RestorePoint2){return;
}var d=document.createElement("div");
d.innerHTML=c.RestorePoint2.HtmlText;
c.RestorePoint2.HtmlText=this._getCleanResizableHandlesFilter().getHtmlContent(d).innerHTML;
},_getResizable:function(c){return new Telerik.Web.UI.Widgets.Resizable(c,{liveResize:true,useTinyHandles:true,ownerDocument:this.get_document()});
},_disposeElementResizableWidget:function(){this._elementsResizableWidget.dispose();
this._elementsResizableWidget=null;
this._selectionOrgBorder=null;
},_reInitResizable:function(c){c.dispose();
this._elementsResizableWidget=null;
this._elementResizingSelectionChangeHandler();
},_getElementForResizing:function(e){var d=null,c=e;
if(e&&e.tagName&&e.tagName.toLowerCase()=="img"){return e;
}while(c&&c!=this.get_contentAreaElement()&&c!=$get(this.get_id()+"Center")){if(c.tagName&&c.tagName.toLowerCase()=="table"){d=c;
break;
}c=c.parentNode;
}return d;
},_markHandlesForContentFilter:function(){var d=this._elementsResizableWidget._handlesCollection;
for(var c in d){d[c]._element.setAttribute("rel","rrWidget");
...
}this.whitespaceTracker.dispose();
...
if(h.style.height!="100%"){this._updateEditorSize(m);
}return;
...
}}},onParentNodeChanged:function(){this.whitespaceTracker.stopTracking();
var c=this.get_html();
...
},getToolState:function(e,d){if(e=="Undo"){return this.get_commandsManager().isUndoAvailable()?b.Editor.CommandStates.Off:b.Editor.CommandStates.Disabled;
}else{if(e=="Redo"){return this.get_commandsManager().isRedoAvailable()?b.Editor.CommandStates.Off:b.Editor.CommandStates.Disabled;
}}if(e=="FormatPainterApply"||e=="FormatPainterClear"){return(this.get_currentFormatting())?b.Editor.CommandStates.Off:b.Editor.CommandStates.Disabled;
}else{var c=b.Editor.UpdateCommandsArray[e];
if(c&&c.getState){return c.getState(this.get_contentWindow(),this,d);
...
}}if(c&e.ConvertTags){d.add(new b.Editor.ConvertTagsFilter());
}if($telerik.isOpera||$telerik.isSafari||$telerik.isChrome){d.add(new b.Editor.CleanResizableHandles());
...
}},_updateEditorSize:function(f,h){var g=this._tableElement;
var d=f?f:g.style.height;
var e=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
if(true==h||d=="auto"){d=g.offsetHeight+"px";
}if(parseInt(d)==0){return;
}d=parseInt(d)+"px";
g.style.height=d;
if(this.isIE){this._fixIeHeight(g,d);
}this._fixMoreSizeIssues();
var c=this.get_element();
if(c.style.height!="auto"){c.style.height=d;
if(!$telerik.isIE7&&!$telerik.isIE8){this._setCenterCellIframeInitialHeight();
}else{this._setElementStyleProperty(e,"height","100%");
}}},_fixIeHeight:function(f,d){if(($telerik.isIE6||$telerik.isIE8||$telerik.isIE7)&&"CSS1Compat"==document.compatMode){var c=(f.offsetHeight-parseInt(d));
if(c>0){var e=(parseInt(f.style.height)-c);
if(e>0){f.style.height=e+"px";
}}}},_fixMoreSizeIssues:function(){var f=(this.get_contentAreaMode()==b.EditorContentAreaMode.Div);
var c=this.get_autoResizeHeight();
var e=this.get_element();
this._clearWrapperPositioning(e);
if(c&&!f){return;
}else{if(c&&this._isFullScreen!=true&&$telerik.isIE){window.setTimeout(Function.createDelegate(this,this._setElementsDimensions),0);
}else{var g=this.get_mainTable();
if(g.offsetWidth==0){return;
}this._setCenterCellIframeInitialHeight();
if(this._isFullScreen){this._setResizeHeight();
}else{if(this.get_contentAreaMode()==b.EditorContentAreaMode.Div){var d=this._calcCenterHeight();
if(d&&d>0){this._setIframeCurrentHeight(d);
}this._setIframeHeight();
}if(""==e.style.minHeight){this._setLowerWrapperMinHeight();
}if(""==e.style.minWidth){this._setLowerWrapperMinWidth();
}if(e.style.width.indexOf("%")<0){this._setLowerWrapperWidth();
}if(f&&c){this._WrapperAutoResizeHeight();
}}}}},_setElementStyleProperty:function(c,d,e){c.style[d]=e;
},_setCenterCellIframeInitialHeight:function(){var c=$get(this.get_id()+"Center"),l=$get(this.get_id()+"Wrapper"),k=l.getElementsByTagName("tr"),f=0,h=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
this._setCenterCellInitialHeight();
this._setIframeAbsolutePosition();
for(var g=0;
g<k.length;
g++){var j=k[g];
if(j!=c.parentNode&&j.parentNode.parentNode==l){f=f+j.offsetHeight;
}}var d=parseInt($telerik.getComputedStyle(c,"border-top-width",0))+parseInt($telerik.getComputedStyle(c,"border-bottom-width",0));
var e=l.offsetHeight-parseInt(d)-f;
this._setIframeStaticPosition();
if(e>=0){this._setElementStyleProperty(c,"height","100%");
this._setElementStyleProperty(h,"height",e+"px");
}if($telerik.isIE8){this._setElementStyleProperty(c,"height","100%");
this._setElementStyleProperty(h,"height","100%");
}if($telerik.isOpera){this._setElementStyleProperty(h,"height",e-1+"px");
}},_setCenterCellInitialHeight:function(){var c=$get(this.get_id()+"Center"),d=$get(this.get_id()+"Wrapper");
if(c.offsetHeight>=parseInt(d.style.height)){this._setElementStyleProperty(c,"height",parseInt(d.style.height)||this.get_element().offsetHeight+"px");
}if($telerik.isIE8){this._setElementStyleProperty(c,"height","100%");
}},_clearWrapperPositioning:function(){var c=this.get_element();
this._setElementStyleProperty(c,"position","");
},_setElementsDimensions:function(){var c=this.get_contentAreaElement();
var d=this.get_element();
this._setWrapperHeight();
if(""==d.style.minHeight){this._setWrapperMinHeight();
}if(""==d.style.minWidth){this._setWrapperMinWidth();
}if(d.style.width.indexOf("%")<0){this._setWrapperWidth();
}this._setContentAreaHeight();
if(""==c.style.minHeight){this._setContentAreaMinHeight();
}if(""==c.style.minWidth){this._setContentAreaMinWidth();
}},_setWrapperHeight:function(){var c=this.get_element();
this._setElementStyleProperty(c,"height","auto");
},_setWrapperMinHeight:function(){var c=this.get_element();
var d=this.get_mainTable();
this._setElementStyleProperty(c,"minHeight",d.offsetHeight+"px");
},_setWrapperWidth:function(){var c=this.get_element();
var d=this.get_mainTable();
this._setElementStyleProperty(c,"width",(d.offsetWidth-6)+"px");
},_setWrapperMinWidth:function(){var c=this.get_element();
var d=this.get_mainTable();
this._setElementStyleProperty(c,"minWidth",d.offsetWidth+"px");
},_setContentAreaHeight:function(){var c=this.get_contentAreaElement();
this._setElementStyleProperty(c,"height","auto");
},_setContentAreaMinHeight:function(){var d=this.get_contentAreaElement();
var c=$get(this.get_id()+"Center");
this._setElementStyleProperty(d,"minHeight",c.offsetHeight+"px");
},_setContentAreaMinWidth:function(){var d=this.get_contentAreaElement();
var c=$get(this.get_id()+"Center");
this._setElementStyleProperty(d,"minWidth",(c.offsetWidth-12)+"px");
},_setContentCellHeight:function(){var c=$get(this.get_id()+"Center");
this._setElementStyleProperty(c,"height","100%");
},_setResizeHeight:function(){this._setWrapperMinDimensions();
if($telerik.isOpera){this._setResizeHeightIsOpera();
}if(this.get_contentAreaMode()==b.EditorContentAreaMode.Div){this._setResizeHeightModes();
}},_setWrapperMinDimensions:function(){var c=this.get_element();
this._setElementStyleProperty(c,"minWidth","");
this._setElementStyleProperty(c,"minHeight","");
},_setResizeHeightIsOpera:function(){var c=$get(this.get_id()+"Center");
var d=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
this._setElementStyleProperty(d,"height","0px");
var e=c.offsetHeight-2;
this._setElementStyleProperty(d,"height",e+"px");
this._setElementStyleProperty(c,"height",e+"px");
},_setResizeHeightModes:function(){var c=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
c.style.height=($telerik.isIE)?"100%":c.parentNode.offsetHeight+"px";
},_calcCenterHeight:function(){var c=$get(this.get_id()+"Center");
this._setIframeAbsolutePosition();
var d=c.offsetHeight-2;
this._setIframeRelativePosition();
return d;
},_setIframeAbsolutePosition:function(){var c=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
this._setElementStyleProperty(c,"position","absolute");
},_setIframeCurrentHeight:function(c){var d=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
this._setElementStyleProperty(d,"height",c+"px");
},_setIframeRelativePosition:function(){var c=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
this._setElementStyleProperty(c,"position","relative");
},_setIframeHeight:function(){var c=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
if(c.offsetHeight>0){if($telerik.isIE&&!$telerik.isIE9Mode){this._setMainTableHeightIE();
}else{var d=this.get_mode();
if(d==Telerik.Web.UI.EditModes.Html){this._setMainTableHeightCorrector();
}else{this._setMainTableHeight();
}}this._mainTableIframeHeight();
}},_setMainTableHeightIE:function(){var c=this.get_mainTable();
this._setElementStyleProperty(c,"height",(parseInt(c.style.height)-6)+"px");
},_setMainTableHeightCorrector:function(){var d=this.get_mainTable();
var c=($telerik.isIE||$telerik.isOpera)?3:4;
this._setElementStyleProperty(d,"height",(parseInt(d.style.height)-c)+"px");
},_setMainTableHeight:function(){var c=this.get_mainTable();
this._setElementStyleProperty(c,"height",this.get_element().style.height);
},_mainTableIframeHeight:function(){var c=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
this._setElementHeight(c,(c.offsetHeight-12)+"px");
},_setElementHeight:function(c,d){this._setElementStyleProperty(c,"height",d);
},_setLowerHeightToElement:function(){var c=$get(this.get_id()+"Center");
var d=c.offsetHeight;
this._setIframeStaticPosition();
if(d<160&&c.offsetHeight>d+2){this._setLowerHeight();
}},_setIframeStaticPosition:function(){var c=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
this._setElementStyleProperty(c,"position","");
},_setLowerHeight:function(){var c=$get(this.get_id()+"Center");
var d=c.offsetHeight;
this._setElementStyleProperty(c,"height",(d-6)+"px");
},_setLowerHeightOpera:function(){this._setLowerIframeHeightOpera();
var c=$get(this.get_id()+"Center");
if(c.style.height=="100%"){this._setElementStyleProperty(c,"height",(2*c.offsetHeight-c.parentNode.parentNode.offsetHeight-13)+"px");
}},_setLowerIframeHeightOpera:function(){var c=this.get_mode()!=b.EditModes.Html?this.get_contentAreaElement():this._getTextIframe();
this._setElementStyleProperty(c,"height","100%");
},_setLowerCenterHeightOpera:function(){var c=$get(this.get_id()+"Center");
this._setElementStyleProperty(c,"height",(2*c.offsetHeight-c.parentNode.parentNode.offsetHeight-13)+"px");
},_setLowerWrapperMinHeight:function(){var c=this.get_element();
var d=this.get_mainTable();
this._setElementStyleProperty(c,"minHeight",d.style.height);
},_setLowerWrapperMinWidth:function(){var c=this.get_element();
var d=this.get_mainTable();
this._setElementStyleProperty(c,"minWidth",d.offsetWidth+"px");
},_setLowerWrapperWidth:function(){var c=this.get_element();
var d=this.get_mainTable();
this._setElementStyleProperty(c,"width",d.offsetWidth+"px");
},_WrapperAutoResizeHeight:function(){var c=this.get_contentAreaElement();
this._setWrapperAutoResizeHeight();
this._setContentAreaAutoResizeHeight();
if(""==c.style.minHeight){this._setContentAreaAutoResizeMinHeight();
}if(""==c.style.minWidth){this._setContentAreaAutoResizeMinWidth();
}},_setWrapperAutoResizeHeight:function(){var c=this.get_element();
this._setElementStyleProperty(c,"height","auto");
},_setContentAreaAutoResizeHeight:function(){var c=this.get_contentAreaElement();
this._setElementStyleProperty(c,"height","auto");
},_setContentAreaAutoResizeMinHeight:function(){var d=this.get_contentAreaElement();
var c=$get(this.get_id()+"Center");
this._setElementStyleProperty(d,"minHeight",c.offsetHeight+"px");
},_setContentAreaAutoResizeMinWidth:function(){var d=this.get_contentAreaElement();
var c=$get(this.get_id()+"Center");
this._setElementStyleProperty(d,"minWidth",(c.offsetWidth-12)+"px");
},setSize:function(k,j){var c=$telerik.isMobileSafari&&this.get_contentAreaMode()==b.EditorContentAreaMode.Iframe&&this._contentAreaWrapper;
if(c){this._contentAreaWrapper.style.height="1px";
}var l=parseInt(k);
var d=parseInt(j);
var g=this.get_mainTable();
if(g&&g.style){g.style.height="";
var f=g.parentNode;
f.style.width=l+"px";
f.style.height=d+"px";
if($telerik.isIE){g.style.height=d+"px";
var h=this;
setTimeout(function(){h._fixIeHeight(g,d);
g=null;
},0);
...
},_removeCursorHolder:function(f){var k=f.keyCode;
var j=!f.ctrlKey&&(k==8||(!((k>=9&&k<=45)||(k>=112&&k<=145)||(k>=91&&k<=93))));
if(!j){return;
}var l=this.get_newLineMode();
if($telerik.isIE&&!$telerik.isIE9Mode&&l==b.EditorNewLineModes.Div){var n=this.getSelection().getRange(),g=n.parentElement(),m=g?b.Editor.Utils.getFirstBlockElementUp(g):null;
if(g&&m&&g.innerHTML=="&nbsp;"&&m.nodeName=="DIV"&&!b.Editor.Utils.isEditorContentArea(m)){var o=g.firstChild;
o.deleteData(o.nodeValue.length-1,1);
}}else{if($telerik.isFirefox){var d=b.Editor.DomRange.toDomRange(this.getSelection().getRange(true)),c=d.commonAncestorContainer,h=b.Editor.Utils.isTextNode(c);
if(!c){return;
}if(h&&c.newLineCommand){$telerik.$(c).remove();
}else{if(l!=b.EditorNewLineModes.Br&&!h&&b.Editor.Utils.getTextContent(c).trim()==""){setTimeout(function(){$telerik.$(c).find("br:first").remove();
},0);
...
}},_onDragStart:function(d){var c=this.get_contentWindow().document;
if($telerik.isIE&&!c.selection){return $telerik.cancelRawEvent(d);
}b.Editor.Utils.storeOriginalPathsAndAttributes(this.get_contentArea());
if($telerik.isFirefox){return;
}var j=this.get_localization()["MoveCommand"]||"Move";
this._pendingMoveCommand=new b.Editor.GenericCommand(j,this.get_contentArea(),this);
this._startRange=null;
if(!d.ctrlKey&&!d.ctrlLeft){this._startRange=c.selection.createRange();
if(this._startRange.length){var h=c.body.createTextRange();
var f=this._startRange.item(0);
if("IMG"==f.tagName&&"A"==f.parentNode.tagName&&f.parentNode.childNodes.length==1){f=f.parentNode;
}b.Editor.Utils.storeOriginalPathsAndAttributes(f);
try{h.moveToElementText(f);
this._startRange=h;
}catch(g){}}}},_onDragEnd:function(d){var c=this.get_contentWindow().document;
if($telerik.isIE&&!c.selection){return $telerik.cancelRawEvent(d);
}b.Editor.Utils.restoreOriginalPathsAndAttributes(this.get_contentArea());
if($telerik.isFirefox){return;
}if(this._pendingMoveCommand){if(this._startRange){var g=c.selection.createRange();
if(g.length){var h=c.body.createTextRange();
try{h.moveToElementText(g.item(0));
}catch(f){}g=h;
}try{if(g.compareEndPoints&&0!=g.compareEndPoints("StartToStart",this._startRange)&&0!=g.compareEndPoints("EndToEnd",this._startRange)){this._startRange.execCommand("Delete",false,null);
}}catch(d){}}this.executeCommand(this._pendingMoveCommand);
...
var l=h(j);
if(l.getSelection||j.getSelection){var k=l.getSelection?l.getSelection():j.getSelection();
k.removeAllRanges();
k.addRange(i);
...
},_calculateEdge:function(q,v){var x=this.range;
var o=this._isBrowserRangeCollapsed();
var k=d(x);
var i=q?"startContainer":"endContainer";
var s=q?"startOffset":"endOffset";
var l=x[i];
var m=x[s]||0;
if(!l){if(x.length){var r=x.item(0);
l=r.parentNode;
m=g.findNodeIndex(r)+(q?0:1);
}else{var j=x.duplicate();
j.collapse(q);
var u=j.parentElement(),p=u.childNodes.length==0;
if(!g.canHaveChildren(u)||g.isTag(u,"textarea")){l=u;
}else{var y=this._createTempNode(k);
var n=q?"StartToStart":"StartToEnd";
do{u.insertBefore(y,y.previousSibling);
j.moveToElementText(y);
}while(y.previousSibling&&j.compareEndPoints(n,x)>0);
l=p?u:y.nextSibling;
}if(!l){if(q&&!o){u.removeChild(y);
var t=u;
while(t==u&&this._moveCursorByCharacter(j)){u=j.parentElement();
}do{u.insertBefore(y,y.previousSibling);
j.moveToElementText(y);
}while(y.previousSibling&&j.compareEndPoints(n,x)>0);
l=y.nextSibling;
}if(!l){var w=y.previousSibling;
if(!v&&w&&g.isTextNode(w)){this[i]=w;
this[s]=w.length||w.data.length||w.nodeValue.length;
}else{if(!this.startContainer){this.startContainer=u;
this.startOffset=g.findNodeIndex(y);
}this.endContainer=u;
this.endOffset=u[u.nodeType===1?"childNodes":"nodeValue"].length-1;
}u.removeChild(y);
return;
}}if(y){u.removeChild(y);
}j.setEndPoint(q?"EndToStart":"EndToEnd",x);
if(g.isTextNode(l)){m=j.text.length;
}else{if(i=="startContainer"&&g.isAncestorOrSelf(l,x.parentElement())||(p&&i=="endContainer")){m=0;
...
g<f.length;
g++){h.push(f[g]);
}},splitRangeEdges:function(){var k=this.nodesToTraverse;
var m=k[0];
var n=$telerik.$(m.parentNode);
var f=k[k.length-1];
var g=$telerik.$(f.parentNode);
var h=this.splitRangeEdge(f,false);
var j=this.splitRangeEdge(m,true);
if(!a.inArray(h,k)){k[k.length-1]=h;
}this.cleanUpAfterSplit(n,g);
this.removeRedundantStart(k,j);
...
}}},cleanUpAfterSplit:function(m,g){var h=this.nodesToTraverse,j=this.range,k=m.find(".__telerik_marker_start")[0],f=g.find(".__telerik_marker_end")[0];
if(k){j.setStartBefore(k);
k.parentNode.removeChild(k);
}if(f){j.setEndBefore(f);
f.parentNode.removeChild(f);
}else{this.range.setEndAfter(h[h.length-1]);
}if(k||f){j.select();
}},removeDuplicatedEnd:function(){var g=this.traversedFragments;
if(g.length<2){return;
}var f=g.length,h=g[f-1],j=h.nodes[0],k=g[f-2],m=k.nodes[k.nodes.length-1];
while(e.isAncestorOf(m,j)){h.removeNodeAt(0);
...
},isEmpty:function(){var h=this.nodes;
for(var f=0;
f<h.length;
f++){var g=h[f];
if(e.isTag(g,"img")||!e.isWhitespaceNode(h[f])){return false;
}}return true;
}};
...
return e;
},_hasSubTree:function(e){return !d.isTextNode(e)&&(d.isAncestorOrSelf(e,this.range.startContainer)||d.isAncestorOrSelf(e,this.range.endContainer));
...
(function() {
    function loadHandler() {
        ...
        hf.value += ';;System.Web.Extensions, Version=4.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35:en-US:c9cbdec3-c810-4e87-846c-fb25a7c08002:ea597d4b:b25378d2;Telerik.Web.UI, Version=2013.3.1015.45, Culture=neutral, PublicKeyToken=121fae78165ba3d4:en-US:7a1d9c2d-6cf2-464f-9ad3-e479c730ac61:16e4e7cd:86526ba7:874f8ea2:5d3f911d:f7645509:24ee1bba:92fe8ea0:fa31b949:dc7e0bd';                                    
