...
<div class="fm-dlg-panel fm-dlg">
	<table style="margin:30px 0px 10px 50px; border-spacing: 5px;">
		<tr>
			<td style="font-size:9pt;" class="req">Name:</td>
			<td style="width:230px" colspan="2"><input id="tbName" class="fm" maxlength="50" /></td>
		</tr>
...
RMC.CreateTableDialog= function(wnd) {
	...
	this.registerFromDatabase= function() {
		...
		this._window.setSize(600, 500);this._window.setCenter();
		this._window.loadURL(FM.Path + "Explorer/TableBuilder/_generator.aspx?sl=1&sourceName="+t);
	}

	this.addFromDB= function() {
		this._window.loadHTML("");
		this._window.setSize(500, 500);this._window.setCenter();
		this._window.loadURL(FM.Path + "Explorer/RMC/TableMenu/sl_SelectFromDB.aspx");
	}
